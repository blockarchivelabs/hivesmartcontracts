...
const reimburseNFTs = async ({ to, toType, symbol, ids, batchSize }) => {
  ...
  for (let i = 0, n = ids.length; i < n; i += 1000) {
    ...
    reservedNfts.push(
      ...nftsInContract.map((x) => api.BigNumber(x._id).toString())
...
/* eslint-disable-next-line object-curly-newline */
const parseAndValidateAirdrop = async ({
  symbol,
  sender,
  senderType,
  list,
  ...
  if (
    api.assert(params.enabledFromTypes.includes(senderType), "invalid fromType")
  ) {
      ...
      if (
        api.assert(
          Array.isArray(list) &&
            list.length > 0 &&
            api.assert(
              list.length <= params.maxTransactionsPerAirdrop,
              "exceeded airdrop transactions limit"
            ) && // Don't even bother if the list is already too long.
            list.every((element, index) => {
              if (typeof element === "object") {
                ...
                if (
                  api.assert(
                    isValidAccountName(to, toType),
                    `invalid account ${to} at index ${index}`
                  ) &&
                  api.assert(
                    Array.isArray(ids) &&
                      ids.length > 0 &&
                      ids.length <= params.maxTransactionsPerAccount &&
                      ids.every(
                        (i) => typeof i === "string" && api.BigNumber(i).gt(0)
                      ),
                    `invalid nft ids array for account ${to} at index ${index}`
                  ) &&
                  api.assert(
                    airdrop.nftIds.length < params.maxTransactionsPerAirdrop,
                    "exceeded airdrop transactions limit"
                  )
                ) {
        ...
        if (
          api.assert(
            !arrayHasDuplicates(airdrop.nftIds),
            "airdrop list contains duplicate nfts"
          )
        ) {
          ...
          const result = await api.db.findOneInTable("nft", instanceTableName, {
            _id: {
              $in: airdrop.nftIds.map((x) => api.BigNumber(x).toNumber()),
            },
            $or: [
              {
                account: { $ne: sender },
          ...
          if (
            ...
            if (
              api.assert(
                Number.isInteger(airdrop.blockNumber) &&
                  airdrop.blockNumber > api.blockNumber,
                "invalid startBlockNumber"
              )
            ) {
              airdrop.totalFee = api
                .BigNumber(params.feePerTransaction)
                .times(airdrop.nftIds.length)
...
const processAirdrop = async (airdrop, batchSize) => {
  const { softFail, symbol, list, nftIds } = airdrop;
  ...
  for (let i = list.length - 1; i >= 0; i -= 1) {
    ...
    if (batch.length > 0) {
      ...
      if (transfer.errors !== undefined) {
        const success = transfer.events
          ? transfer.events.map((x) => x.data.id)
          : [];
  ...
  nftIds.splice(
    0,
    nftIds.length,
    ...nftIds.filter((x) => failed.includes(x) || !processed.includes(x))
  );
...
actions.updateParams = async (payload) => {
  if (api.assert(api.sender === api.owner, "not authorized")) {
    ...
    if (maxTransactionsPerAirdrop !== undefined) {
      if (
        api.assert(
          Number.isInteger(maxTransactionsPerAirdrop) &&
            maxTransactionsPerAirdrop > 0,
          "invalid maxTransactionsPerAirdrop"
        )
      ) {
    ...
    if (maxTransactionsPerAccount !== undefined) {
      if (
        api.assert(
          Number.isInteger(maxTransactionsPerAccount) &&
            maxTransactionsPerAccount > 0 &&
            maxTransactionsPerAccount <= params.maxTransactionsPerAirdrop,
          "invalid maxTransactionsPerAccount"
        )
      ) {
    ...
    if (maxAirdropsPerBlock !== undefined) {
      if (
        api.assert(
          Number.isInteger(maxAirdropsPerBlock) && maxAirdropsPerBlock > 0,
    ...
    if (processingBatchSize !== undefined) {
      if (
        api.assert(
          Number.isInteger(processingBatchSize) && processingBatchSize > 0,
          "invalid processingBatchSize"
        )
      ) {
    ...
    if (enabledFromTypes !== undefined) {
      if (
        api.assert(
          Array.isArray(enabledFromTypes) &&
            enabledFromTypes.length > 0 &&
            enabledFromTypes.every((x) => ALLOWED_FROM_TYPES.includes(x)),
          "invalid enabledFromTypes"
        )
      ) {
...
actions.newAirdrop = async (payload) => {
  ...
  if (
    ...
    const senderType =
      callingContractInfo === undefined || fromType !== "contract"
        ? "user"
        : "contract";
    const sender =
      senderType === "user" ? api.sender : callingContractInfo.name;
    const params = await api.db.findOne("params", {});
    ...
    if (airdrop.isValid) {
      ...
      const tokensTable =
        senderType === "contract" ? "contractsBalances" : "balances";
      ...
      if (
        api.assert(
          utilityToken &&
            api.BigNumber(utilityToken.balance).gte(airdrop.totalFee),
  ...
  if (api.assert(api.sender === "null", `not authorized: ${api.sender}`)) {
    ...
    if (pendingAirdrops.length > 0) {
      const batchSize = Math.floor(
        params.maxTransactionsPerBlock / pendingAirdrops.length
      );

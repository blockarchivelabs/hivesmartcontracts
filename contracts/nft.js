...
actions.updateParams = async (payload) => {
  ...
  if (
    nftCreationFee &&
    typeof nftCreationFee === "string" &&
    !api.BigNumber(nftCreationFee).isNaN() &&
    api.BigNumber(nftCreationFee).gte(0)
  ) {
  ...
  if (
    dataPropertyCreationFee &&
    typeof dataPropertyCreationFee === "string" &&
    !api.BigNumber(dataPropertyCreationFee).isNaN() &&
    api.BigNumber(dataPropertyCreationFee).gte(0)
  ) {
    params.dataPropertyCreationFee = dataPropertyCreationFee;
  }
  if (
    enableDelegationFee &&
    typeof enableDelegationFee === "string" &&
    !api.BigNumber(enableDelegationFee).isNaN() &&
    api.BigNumber(enableDelegationFee).gte(0)
  ) {
...
const isValidContractLength = (contract) =>
  contract.length >= 3 && contract.length <= 50;
...
const isValidDataProperties = (from, fromType, nft, properties) => {
  ...
  if (
    !api.assert(
      propertyCount <= nftPropertyCount,
  ...
  for (const [name, data] of Object.entries(properties)) {
    ...
    if (
      api.assert(
        name &&
          typeof name === "string" &&
          api.validator.isAlphanumeric(name) &&
          name.length > 0 &&
          name.length <= 25,
            ...
            data !== undefined &&
              ...
              (typeof data === propertySchema.type ||
                (propertySchema.type === "number" &&
                  typeof data === "string" &&
                  !api.BigNumber(data).isNaN())),
            `data property type mismatch: expected ${
              propertySchema.type
            } but got ${typeof data} for property ${name}`
          ) &&
          api.assert(
            typeof data !== "string" || data.length <= MAX_DATA_PROPERTY_LENGTH,
            `string property max length is ${MAX_DATA_PROPERTY_LENGTH} characters`
          ) &&
          api.assert(
            (fromType === "contract" &&
              propertySchema.authorizedEditingContracts.includes(from)) ||
              (fromType === "user" &&
                propertySchema.authorizedEditingAccounts.includes(from)),
  ...
  try {
    ...
    for (let i = 0; i < arr.length; i += 1) {
      ...
      if (
        ...
        if (
          api.assert(
            instanceCount <= MAX_NUM_NFTS_OPERABLE,
            `cannot operate on more than ${MAX_NUM_NFTS_OPERABLE} NFT instances at once`
          )
        ) {
...
const isValidTokenBasket = async (
  basket,
  balanceTableName,
  accountName,
  feeSymbol,
      ...
      if (
        ...
        if (token) {
          if (
            ...
            const finalQuantity =
              symbol === feeSymbol
                ? calculateBalance(quantity, feeQuantity, token.precision, true)
                : quantity;
            const basketTokenBalance = await api.db.findOneInTable(
              "tokens",
              balanceTableName,
              { account: accountName, symbol }
            );
            if (
              basketTokenBalance &&
              api.BigNumber(basketTokenBalance.balance).gte(finalQuantity)
            ) {
              validContents = true;
            }
          }
...
const transferAndVerifyNfts = async (
  from,
  fromType,
  to,
  toType,
...
actions.updateName = async (payload) => {
  ...
  if (
    api.assert(
      symbol && typeof symbol === "string" && name && typeof name === "string",
      "invalid params"
    ) &&
    api.assert(
...
actions.updateOrgName = async (payload) => {
  ...
  if (
    ...
    api.assert(
      api.validator.isAlphanumeric(api.validator.blacklist(orgName, " ")) &&
        orgName.length > 0 &&
        orgName.length <= 50,
...
actions.updateProductName = async (payload) => {
  ...
  if (
    ...
    api.assert(
      api.validator.isAlphanumeric(api.validator.blacklist(productName, " ")) &&
...
actions.addAuthorizedIssuingAccounts = async (payload) => {
  ...
  if (
    ...
    api.assert(
      accounts.length <= MAX_NUM_AUTHORIZED_ISSUERS,
      `cannot have more than ${MAX_NUM_AUTHORIZED_ISSUERS} authorized issuing accounts`
    )
  ) {
    ...
    if (api.assert(validContents, "invalid account list")) {
      ...
      if (nft) {
        ...
        if (
          ...
          api.assert(
            !containsDuplicates(sanitizedList),
            "cannot add the same account twice"
          ) &&
          api.assert(
            nft.authorizedIssuingAccounts.length + sanitizedList.length <=
              MAX_NUM_AUTHORIZED_ISSUERS,
            `cannot have more than ${MAX_NUM_AUTHORIZED_ISSUERS} authorized issuing accounts`
          )
        ) {
          const finalAccountList =
...
actions.addAuthorizedIssuingContracts = async (payload) => {
  ...
  if (
    ...
    api.assert(
      contracts.length <= MAX_NUM_AUTHORIZED_ISSUERS,
      `cannot have more than ${MAX_NUM_AUTHORIZED_ISSUERS} authorized issuing contracts`
    )
  ) {
    ...
    if (api.assert(validContents, "invalid contract list")) {
      ...
      if (nft) {
        ...
        if (
          ...
          api.assert(
            nft.authorizedIssuingContracts.length + sanitizedList.length <=
              MAX_NUM_AUTHORIZED_ISSUERS,
            `cannot have more than ${MAX_NUM_AUTHORIZED_ISSUERS} authorized issuing contracts`
          )
        ) {
          const finalContractList =
            nft.authorizedIssuingContracts.concat(sanitizedList);
...
actions.removeAuthorizedIssuingAccounts = async (payload) => {
  ...
  if (
    ...
    api.assert(
      accounts.length <= MAX_NUM_AUTHORIZED_ISSUERS,
      `cannot remove more than ${MAX_NUM_AUTHORIZED_ISSUERS} authorized issuing accounts`
    )
  ) {
    const validContents = isValidAccountsArray(accounts);
    if (api.assert(validContents, "invalid account list")) {
      ...
      if (nft) {
        if (api.assert(nft.issuer === api.sender, "must be the issuer")) {
          ...
          const finalAccountList = nft.authorizedIssuingAccounts.filter(
            (currentValue) => {
...
actions.removeAuthorizedIssuingContracts = async (payload) => {
  ...
  if (
    ...
    api.assert(
      contracts.length <= MAX_NUM_AUTHORIZED_ISSUERS,
      `cannot remove more than ${MAX_NUM_AUTHORIZED_ISSUERS} authorized issuing contracts`
    )
  ) {
    const validContents = isValidContractsArray(contracts);
    if (api.assert(validContents, "invalid contract list")) {
      ...
      if (nft) {
        if (api.assert(nft.issuer === api.sender, "must be the issuer")) {
          // build final list, removing entries that are both in the input list & current authorized list
          const finalContractList = nft.authorizedIssuingContracts.filter(
            (currentValue) => {
...
actions.enableDelegation = async (payload) => {
  ...
  if (
    ...
    if (
      ...
      api.assert(nft.issuer === api.sender, "must be the issuer") &&
      api.assert(
        nft.delegationEnabled === undefined || nft.delegationEnabled === false,
...
actions.updatePropertyDefinition = async (payload) => {
  const { symbol, name, newName, type, isReadOnly, isSignedWithActiveKey } =
    payload;
    ...
    api.assert(
      newName === undefined ||
        (typeof newName === "string" &&
          api.validator.isAlphanumeric(newName) &&
          newName.length > 0 &&
          newName.length <= 25),
  ...
  if (
    ...
    api.assert(
      symbol &&
        ...
        typeof name === "string" &&
        (isReadOnly === undefined || typeof isReadOnly === "boolean") &&
        (authorizedEditingAccounts === undefined ||
          (authorizedEditingAccounts &&
            typeof authorizedEditingAccounts === "object" &&
            Array.isArray(authorizedEditingAccounts))) &&
        (authorizedEditingContracts === undefined ||
          (authorizedEditingContracts &&
            typeof authorizedEditingContracts === "object" &&
            Array.isArray(authorizedEditingContracts))) &&
        type &&
        typeof type === "string",
      "invalid params"
    ) &&
    ...
    if (nft) {
      if (
        ...
        if (propertyCount >= 3) {
          ...
          const authorizedCreation = api
            .BigNumber(dataPropertyCreationFee)
            .lte(0)
            ? true
            : utilityTokenBalance &&
              api
                .BigNumber(utilityTokenBalance.balance)
                .gte(dataPropertyCreationFee);
          ...
          if (
            ...
            if (api.BigNumber(dataPropertyCreationFee).gt(0)) {
              const res = await api.executeSmartContract("tokens", "transfer", {
                to: "null",
                symbol: UTILITY_TOKEN_SYMBOL,
                quantity: dataPropertyCreationFee,
                isSignedWithActiveKey,
              });
              // check if the tokens were sent
              if (
        ...
        const initialAccountList =
          authorizedEditingAccounts === undefined ? [api.sender] : [];
        ...
        if (authorizedEditingAccounts || authorizedEditingContracts) {
          await actions.setPropertyPermissions({
            symbol,
            name,
            accounts: authorizedEditingAccounts,
            contracts: authorizedEditingContracts,
            isSignedWithActiveKey,
          });
        }
        return true;
...
actions.setPropertyPermissions = async (payload) => {
  ...
  if (
    ...
    api.assert(
      accounts === undefined || accounts.length <= MAX_NUM_AUTHORIZED_ISSUERS,
      `cannot have more than ${MAX_NUM_AUTHORIZED_ISSUERS} authorized accounts`
    ) &&
    api.assert(
      contracts === undefined || contracts.length <= MAX_NUM_AUTHORIZED_ISSUERS,
      `cannot have more than ${MAX_NUM_AUTHORIZED_ISSUERS} authorized contracts`
    ) &&
    api.assert(
      accounts === undefined || isValidAccountsArray(accounts),
      "invalid account list"
    ) &&
    api.assert(
      contracts === undefined || isValidContractsArray(contracts),
    ...
    if (nft) {
      if (
        ...
        if (accounts) {
          sanitizedAccountList = accounts.map((account) =>
        ...
        if (
          api.assert(
            accounts === undefined || !containsDuplicates(sanitizedAccountList),
            "cannot add the same account twice"
          ) &&
          api.assert(
            contracts === undefined ||
              !containsDuplicates(sanitizedContractList),
            "cannot add the same contract twice"
          )
        ) {
          let shouldUpdate = false;
          if (accounts) {
            nft.properties[name].authorizedEditingAccounts =
              sanitizedAccountList;
            shouldUpdate = true;
          }
          if (contracts) {
            nft.properties[name].authorizedEditingContracts =
              sanitizedContractList;
            shouldUpdate = true;
          }
          if (shouldUpdate) {
            await api.db.update("nfts", nft);
...
actions.setGroupBy = async (payload) => {
  const { symbol, properties, isSignedWithActiveKey } = payload;
  ...
  if (
    ...
    if (nft) {
      ...
      if (
        api.assert(nft.issuer === api.sender, "must be the issuer") &&
        api.assert(
          nft.groupBy === undefined || nft.groupBy.length === 0,
          "list is already set"
        ) &&
        api.assert(
          properties.length <= nftPropertyCount,
          "cannot set more data properties than NFT has"
        ) &&
        api.assert(
          !containsDuplicates(properties),
        ...
        for (let i = 0; i < properties.length; i += 1) {
          ...
          if (
            !api.assert(
              name && typeof name === "string" && name in nft.properties,
...
actions.setProperties = async (payload) => {
  const { symbol, fromType, nfts, callingContractInfo } = payload;
  ...
  if (
    api.assert(
      nfts &&
        ...
        Array.isArray(nfts) &&
        finalFromType &&
        typeof finalFromType === "string" &&
        types.includes(finalFromType) &&
        symbol &&
        typeof symbol === "string" &&
        (callingContractInfo ||
          (callingContractInfo === undefined && finalFromType === "user")),
      "invalid params"
    ) &&
    api.assert(
      nfts.length <= MAX_NUM_NFTS_EDITABLE,
      `cannot set properties on more than ${MAX_NUM_NFTS_EDITABLE} NFT instances at once`
    )
  ) {
    const finalFrom =
      finalFromType === "user" ? api.sender : callingContractInfo.name;
    // check if the NFT exists
    const nft = await api.db.findOne("nfts", { symbol });
    ...
    if (api.assert(nft !== null, "symbol does not exist")) {
      ...
      for (let i = 0; i < nfts.length; i += 1) {
        ...
        const nftInstance = await api.db.findOne(instanceTableName, {
          _id: api.BigNumber(id).toNumber(),
        });
        if (api.assert(nftInstance !== null, "nft instance does not exist")) {
          ...
          for (const [name, data] of Object.entries(properties)) {
            ...
            if (propertySchema.isReadOnly) {
              ...
              if (
                api.assert(
                  !(name in nftInstance.properties),
...
actions.burn = async (payload) => {
  const { fromType, nfts, isSignedWithActiveKey, callingContractInfo } =
    payload;
  const types = ["user", "contract"];
        ...
        for (let j = 0; j < ids.length; j += 1) {
          ...
          if (nftInstance) {
            ...
            if (nftInstance.lockedNfts && nftInstance.lockedNfts.length > 0) {
              ...
              containerCount += 1;
              if (
                containerCount > MAX_NUM_CONTAINER_NFTS_OPERABLE ||
                !isFirstInstanceContainer
              ) {
            ...
            if (
              nftInstance.account === finalFrom &&
              ((nftInstance.ownedBy === "u" && finalFromType === "user") ||
                (nftInstance.ownedBy === "c" &&
                  finalFromType === "contract")) &&
              ...
              // eslint-disable-next-line no-restricted-syntax
              for (const [locksymbol, quantity] of Object.entries(
                nftInstance.lockedTokens
              )) {
                const res = await api.transferTokens(
                  finalFrom,
                  locksymbol,
                  quantity,
                  finalFromType
              ...
              api.assert(
                isTransferSuccess,
                `unable to release locked tokens in: ${symbol}, id ${id}`
              );
              // release any locked NFT instances back to the owning account
              const origLockNfts =
                nftInstance.lockedNfts && nftInstance.lockedNfts.length > 0
                  ? nftInstance.lockedNfts
                  : [];
              if (
                isTransferSuccess &&
                nftInstance.lockedNfts &&
                nftInstance.lockedNfts.length > 0
              ) {
                const res = await transferAndVerifyNfts(
                  CONTRACT_NAME,
                  "contract",
                  finalFrom,
                  finalFromType,
                ...
                api.assert(
                  isTransferSuccess,
                  `unable to release locked NFT instances in: ${symbol}, id ${id}`
                );
              }
              ...
              if (isTransferSuccess) {
                api.emit("burn", {
                  account: finalFrom,
                  ownedBy: origOwnedBy,
                  unlockedTokens: origLockTokens,
                  unlockedNfts: origLockNfts,
                  symbol,
                  id,
                });
              }
  ...
  if (
    ...
    api.assert(
      finalFromType &&
        ...
        typeof to === "string" &&
        finalToType &&
        typeof finalToType === "string" &&
        types.includes(finalToType) &&
        (callingContractInfo ||
          (callingContractInfo === undefined && finalFromType === "user")) &&
        nfts &&
        typeof nfts === "object" &&
    ...
    const finalTo =
      finalToType === "user" ? to.trim().toLowerCase() : to.trim();
    const toValid =
      finalToType === "user"
        ? isValidHiveAccountLength(finalTo)
        : isValidContractLength(finalTo);
    const finalFrom =
      finalFromType === "user" ? api.sender : callingContractInfo.name;

    if (
      api.assert(toValid, "invalid to") &&
      api.assert(
        !(finalToType === finalFromType && finalTo === finalFrom),
        "cannot transfer to self"
      ) &&
      api.assert(
        !(finalToType === "user" && finalTo === "null"),
      ...
      for (let i = 0; i < nfts.length; i += 1) {
        ...
        if (nft) {
          if (
            api.assert(
              nft.delegationEnabled === true,
              `delegation not enabled for ${symbol}`
            )
          ) {
            // eslint-disable-next-line prefer-template
            const instanceTableName = symbol + "instances";
              ...
              if (nftInstance) {
                ...
                if (
                  ...
                  await api.executeSmartContract(
                    "mining",
                    "handleNftDelegationChange",
                    { symbol, nft: nftInstance, add: true }
                  );
                }
              }
            }
      ...
      if (nft) {
        if (
          ...
          const cooldownPeriodMillisec =
            nft.undelegationCooldown * 24 * 3600 * 1000;
            ...
            if (nftInstance) {
              ...
              if (
                ...
                api.emit("undelegateStart", {
                  from: nftInstance.delegatedTo.account,
                  fromType: nftInstance.delegatedTo.ownedBy,
                  symbol,
                  id,
                });
                await api.executeSmartContract(
                  "mining",
                  "handleNftDelegationChange",
                  { symbol, nft: nftInstance, add: false }
                );
              }
            }
          }
...
actions.create = async (payload) => {
  ...
  const authorizedCreation = api.BigNumber(nftCreationFee).lte(0)
    ? true
    : utilityTokenBalance &&
      api.BigNumber(utilityTokenBalance.balance).gte(nftCreationFee);
  ...
  if (
    ...
    api.assert(
      name &&
        ...
        (authorizedIssuingAccounts === undefined ||
          (authorizedIssuingAccounts &&
            typeof authorizedIssuingAccounts === "object" &&
            Array.isArray(authorizedIssuingAccounts))) &&
        (authorizedIssuingContracts === undefined ||
          (authorizedIssuingContracts &&
            typeof authorizedIssuingContracts === "object" &&
            Array.isArray(authorizedIssuingContracts))) &&
        (maxSupply === undefined ||
          (maxSupply &&
    ...
    if (
      api.assert(
        api.validator.isAlpha(symbol) &&
          ...
          symbol.length > 0 &&
          symbol.length <= MAX_SYMBOL_LENGTH,
        `invalid symbol: uppercase letters only, max length of ${MAX_SYMBOL_LENGTH}`
      ) &&
      api.assert(
      ...
      api.assert(
        orgName === undefined ||
          (api.validator.isAlphanumeric(
            api.validator.blacklist(orgName, " ")
          ) &&
            orgName.length > 0 &&
            orgName.length <= 50),
        "invalid org name: letters, numbers, whitespaces only, max length of 50"
      ...
      api.assert(
        productName === undefined ||
          (api.validator.isAlphanumeric(
            api.validator.blacklist(productName, " ")
          ) &&
            productName.length > 0 &&
            productName.length <= 50),
        "invalid product name: letters, numbers, whitespaces only, max length of 50"
      ...
      api.assert(
        maxSupply === undefined || api.BigNumber(maxSupply).gt(0),
      ...
      if (api.assert(nft === null, "symbol already exists")) {
        ...
        const initialAccountList =
          authorizedIssuingAccounts === undefined ? [api.sender] : [];
        ...
        if (!(authorizedIssuingAccounts === undefined)) {
          await actions.addAuthorizedIssuingAccounts({
            accounts: authorizedIssuingAccounts,
            symbol,
            isSignedWithActiveKey,
          });
        }
        if (!(authorizedIssuingContracts === undefined)) {
          await actions.addAuthorizedIssuingContracts({
            contracts: authorizedIssuingContracts,
            symbol,
            isSignedWithActiveKey,
          });
        }
  ...
  if (
    ...
    api.assert(
      symbol &&
        ...
        types.includes(finalToType) &&
        feeSymbol &&
        typeof feeSymbol === "string" &&
        feeSymbol in nftIssuanceFee &&
        (properties === undefined ||
          (properties && typeof properties === "object")) &&
        (lockTokens === undefined ||
          (lockTokens && typeof lockTokens === "object")) &&
        (lockNfts === undefined ||
          (lockNfts &&
            typeof lockNfts === "object" &&
            Array.isArray(lockNfts))),
      "invalid params"
    ) &&
    ...
    const balanceTableName =
      finalFromType === "user" ? "balances" : "contractsBalances";
    if (api.assert(toValid, "invalid to")) {
      // check if the NFT and fee token exist
      const nft = await api.db.findOne("nfts", { symbol });
      const feeToken = await api.db.findOneInTable("tokens", "tokens", {
        symbol: feeSymbol,
      });
      ...
      if (
        ...
        if (
          api.assert(
            (finalFromType === "contract" &&
              nft.authorizedIssuingContracts.includes(finalFrom)) ||
              (finalFromType === "user" &&
                nft.authorizedIssuingAccounts.includes(finalFrom)),
            "not allowed to issue tokens"
          ) &&
          api.assert(
            nft.maxSupply === 0 || nft.supply < nft.maxSupply,
            "max supply limit reached"
          )
        ) {
          // calculate the cost of issuing this NFT
          const propertyCount = Object.keys(nft.properties).length;
          const propertyFee = api
            .BigNumber(nftIssuanceFee[feeSymbol])
            .multipliedBy(propertyCount); // extra fees per property
          const issuanceFee = calculateBalance(
            nftIssuanceFee[feeSymbol],
            propertyFee,
            feeToken.precision,
            true
          ); // base fee + property fees
          const feeTokenBalance = await api.db.findOneInTable(
            "tokens",
            balanceTableName,
            { account: finalFrom, symbol: feeSymbol }
          );
          const authorizedCreation = api.BigNumber(issuanceFee).lte(0)
            ? true
            : feeTokenBalance &&
              api.BigNumber(feeTokenBalance.balance).gte(issuanceFee);
          // sanity checks on any tokens the issuer wants to lock up in this NFT
          if (lockTokens) {
            const isLockValid = await isValidTokenBasket(
              lockTokens,
              balanceTableName,
              finalFrom,
              feeSymbol,
          ...
          if (
            ...
            if (api.BigNumber(issuanceFee).gt(0)) {
              if (finalFromType === "contract") {
                const res = await api.transferTokensFromCallingContract(
                  "null",
                  feeSymbol,
                  issuanceFee,
                  "user"
                ...
                if (
                  !api.assert(
                    ...
                    "unable to transfer issuance fee"
                  )
                ) {
                  return false;
                }
...
actions.issueMultiple = async (payload) => {
  const { instances, isSignedWithActiveKey, callingContractInfo } = payload;
  ...
  if (
    ...
    api.assert(
      instances && typeof instances === "object" && Array.isArray(instances),
      "invalid params"
    ) &&
    api.assert(
      instances.length <= MAX_NUM_NFTS_ISSUABLE,
      `cannot issue more than ${MAX_NUM_NFTS_ISSUABLE} NFT instances at once`
    )
  ) {
    ...
    if (
      api.assert(
        containerCount <= MAX_NUM_CONTAINER_NFTS_OPERABLE,
        `cannot issue more than ${MAX_NUM_CONTAINER_NFTS_OPERABLE} container NFT instances at once`
      ) &&
      api.assert(
        containerCount === 0 || containerCount === instances.length,

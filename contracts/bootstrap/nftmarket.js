...
const updateTradesHistory = async (
  ...
  // clean history
  let tradesToDelete = await api.db.find(historyTableName, {
    priceSymbol,
    timestamp: {
  ...
  while (nbTradesToDelete > 0) {
    ...
    tradesToDelete = await api.db.find(historyTableName, {
      priceSymbol,
      timestamp: {
        $lt: timestampMinus24hrs,
      },
...
actions.buy = async (payload) => {
  const { symbol, nfts, marketAccount, isSignedWithActiveKey } = payload;

...
const VERIFIED_ISSUERS = ["comments", "mining", "tokenfunds"];
...
actions.updateParams = async (payload) => {
  ...
  if (
    tokenCreationFee &&
    typeof tokenCreationFee === "string" &&
    !api.BigNumber(tokenCreationFee).isNaN() &&
    api.BigNumber(tokenCreationFee).gte(0)
  ) {
  ...
  if (
    enableStakingFee &&
    typeof enableStakingFee === "string" &&
    !api.BigNumber(enableStakingFee).isNaN() &&
    api.BigNumber(enableStakingFee).gte(0)
  ) {
...
actions.create = async (payload) => {
  ...
  const authorizedCreation =
    api.BigNumber(tokenCreationFee).lte(0) || heAccounts[api.sender] === 1
      ? true
      : utilityTokenBalance &&
  ...
  if (
    ...
    if (
      ...
      api.assert(
        heAccounts[api.sender] === 1 || symbol.indexOf("SWAP") === -1,
        "invalid symbol: not allowed to use SWAP"
      ) &&
      api.assert(
        heAccounts[api.sender] === 1 || symbol.indexOf("ETH") === -1,
        "invalid symbol: not allowed to use ETH"
      ) &&
      api.assert(
        heAccounts[api.sender] === 1 || symbol.indexOf("BSC") === -1,
        "invalid symbol: not allowed to use BSC"
      ) &&
      api.assert(
        heAccounts[api.sender] === 1 || symbol.indexOf(".") === -1,
      ...
      if (api.assert(token === null, "symbol already exists")) {
        ...
        if (
          api.BigNumber(tokenCreationFee).gt(0) &&
          heAccounts[api.sender] === undefined
        ) {
...
actions.issue = async (payload) => {
  const { to, symbol, quantity, isSignedWithActiveKey, callingContractInfo } =
    payload;

  const fromVerifiedContract =
    api.sender === "null" &&
    VERIFIED_ISSUERS.indexOf(callingContractInfo.name) !== -1;
  if (
    fromVerifiedContract ||
    (api.assert(
      isSignedWithActiveKey === true,
      "you must use a custom_json signed with your active key"
    ) &&
    ...
    if (
      ...
      api.assert(
        fromVerifiedContract || token.issuer === api.sender,
  ...
  if (
    ...
    if (api.assert(finalTo !== api.sender, "cannot transfer to self")) {
      ...
      if (
        ...
        api.assert(
          blacklist[finalTo] === undefined,
          `not allowed to send to ${finalTo}`
        )
      ) {
        const token = await api.db.findOne("tokens", { symbol });
...
actions.transferToContract = async (payload) => {
  const { from, to, symbol, quantity, isSignedWithActiveKey } = payload;

  const finalFrom =
    from === undefined || api.sender !== "null" ? api.sender : from;
...
actions.transferFromContract = async (payload) => {
  ...
  if (api.assert(api.sender === "null", "not authorized")) {
    const { from, to, symbol, quantity, type } = payload;
  ...
  await findAndProcessAll(
    ...
    async (pendingUnstake) => {
      if (pendingUnstake.numberTransactionsLeft > 1) {
        const tokensToRelease = api
          .BigNumber(pendingUnstake.quantity)
          .dividedBy(token.numberTransactions)
          .toFixed(token.precision, api.BigNumber.ROUND_DOWN);
        availableStakeBalance = availableStakeBalance
          .minus(pendingUnstake.quantityLeft)
...
const startUnstake = async (account, token, quantity) => {
  ...
  if (
    ...
    const nextTokensToRelease =
      token.numberTransactions > 1
        ? api
            .BigNumber(quantity)
            .dividedBy(token.numberTransactions)
            .toFixed(token.precision, api.BigNumber.ROUND_DOWN)
    ...
    if (
      ...
      await api.executeSmartContract("tokenfunds", "updateProposalApprovals", {
        account,
        token,
      });
    }
          ...
          if (api.assert(balanceFrom !== null, "balanceFrom does not exist")) {
            ...
            if (
              ...
              balanceTo.pendingUndelegations = calculateBalance(
                balanceTo.pendingUndelegations,
                quantity,
                token.precision,
                true
              );

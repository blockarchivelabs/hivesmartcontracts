    ...
    if (!params.updateIndex) {
      ...
      for (let i = 0; i < resetProposals.length; i += 1) {
        ...
        for (let j = 0; j < propApprovals.length; j += 1) {
          ...
          const approvalAcct = resetAccounts.find(
            (x) => x.account === approval.from
          );
          const approvalAcctWgt = approvalAcct.weights.find(
            (x) => x.symbol === propFund.voteToken
          );
...
actions.updateParams = async (payload) => {
  ...
  if (dtfCreationFee) {
    if (
      !api.assert(
        typeof dtfCreationFee === "string" &&
          !api.BigNumber(dtfCreationFee).isNaN() &&
          api.BigNumber(dtfCreationFee).gte(0),
        "invalid dtfCreationFee"
      )
    )
      return;
  ...
  if (dtfUpdateFee) {
    if (
      !api.assert(
        typeof dtfUpdateFee === "string" &&
          !api.BigNumber(dtfUpdateFee).isNaN() &&
          api.BigNumber(dtfUpdateFee).gte(0),
        "invalid dtfUpdateFee"
      )
    )
      return;
  ...
  if (dtfTickHours) {
    if (
      !api.assert(
        typeof dtfTickHours === "string" &&
          api.BigNumber(dtfTickHours).isInteger() &&
          api.BigNumber(dtfTickHours).gte(1),
        "invalid dtfTickHours"
      )
    )
      return;
  ...
  if (maxDtfsPerBlock) {
    if (
      !api.assert(
        typeof maxDtfsPerBlock === "string" &&
          api.BigNumber(maxDtfsPerBlock).isInteger() &&
          api.BigNumber(maxDtfsPerBlock).gte(1),
        "invalid maxDtfsPerBlock"
      )
    )
      return;
  ...
  if (maxAccountApprovals) {
    if (
      !api.assert(
        typeof maxAccountApprovals === "string" &&
          api.BigNumber(maxAccountApprovals).isInteger() &&
          api.BigNumber(maxAccountApprovals).gte(1),
        "invalid maxDtfsPerBlock"
      )
    )
      return;
...
function validateTokens(payTokenObj, voteTokenObj) {
  if (
    !api.assert(
      payTokenObj &&
        (payTokenObj.issuer === api.sender ||
          // eslint-disable-next-line no-template-curly-in-string
          (payTokenObj.symbol === "'${CONSTANTS.UTILITY_TOKEN_SYMBOL}$'" &&
            api.sender === api.owner)),
      "must be issuer of payToken"
    )
  )
    return false;
  if (
    !api.assert(
      voteTokenObj && voteTokenObj.stakingEnabled,
      "voteToken must have staking enabled"
    )
  )
    return false;
  return true;
...
function validateDateRange(startDate, endDate, maxDays) {
  if (
    !api.assert(
      validateDateTime(startDate) && validateDateTime(endDate),
      "invalid datetime format: YYYY-MM-DDThh:mm:ss.sssZ"
    )
  )
    return false;
  const now = new Date(`${api.steemBlockTimestamp}.000Z`);
  const start = new Date(startDate);
  const end = new Date(endDate);
  if (
    !api.assert(
      api
        .BigNumber(start.getTime())
        .lt(api.BigNumber(end.getTime()).minus(86400 * 1000)),
      "dates must be at least 1 day apart"
    ) ||
    !api.assert(
      api
        .BigNumber(start.getTime())
        .gt(api.BigNumber(now.getTime()).plus(86400 * 1000)),
  ...
  const rangeDays = range
    ...
    .toFixed(0, api.BigNumber.ROUND_CEIL);
  if (
    !api.assert(
      api.BigNumber(rangeDays).lte(maxDays),
      "date range exceeds DTF maxDays"
    )
  )
    return false;
  return true;
...
function validateDateChange(proposal, newDate, maxDays) {
  ...
  if (
    !api.assert(
      api
        .BigNumber(start.getTime())
        .lt(api.BigNumber(repl.getTime()).minus(86400 * 1000)),
      "dates must be at least 1 day apart"
    )
  )
    return false;
...
async function updateProposalWeight(
  id,
  deltaApprovalWeight,
  deltaToken = null
) {
  ...
  if (proposal && validatePending(proposal)) {
    ...
    proposal.approvalWeight = {
      $numberDecimal: api
        .BigNumber(proposal.approvalWeight.$numberDecimal)
...
actions.createFund = async (payload) => {
  ...
  const authorizedCreation =
    api.BigNumber(dtfCreationFee).lte(0) || api.sender === api.owner
      ? true
      : utilityTokenBalance &&
        api.BigNumber(utilityTokenBalance.balance).gte(dtfCreationFee);
  ...
  if (
    ...
    api.assert(
      typeof voteThreshold === "string" && api.BigNumber(voteThreshold).gt(0),
      "invalid voteThreshold: greater than 0"
    ) &&
    api.assert(
      typeof maxDays === "string" &&
        api.BigNumber(maxDays).isInteger() &&
        api.BigNumber(maxDays).gt(0) &&
        api.BigNumber(maxDays).lte(730),
      "invalid maxDays: integer between 1 and 730"
    ) &&
    api.assert(
      typeof maxAmountPerDay === "string" &&
        api.BigNumber(maxAmountPerDay).gt(0),
      "invalid maxAmountPerDay: greater than 0"
    )
  ) {
    if (proposalFee) {
      if (
        !api.assert(
          typeof proposalFee === "object" &&
            typeof proposalFee.method === "string" &&
            FeeMethod.indexOf(proposalFee.method) !== -1 &&
            typeof proposalFee.symbol === "string" &&
            typeof proposalFee.amount === "string" &&
            api.BigNumber(proposalFee.amount).gt(0),
          "invalid proposalFee"
        )
      )
        return;
      const feeTokenObj = await api.db.findOneInTable("tokens", "tokens", {
        symbol: proposalFee.symbol,
      });
      if (
        !api.assert(
          feeTokenObj &&
            api.BigNumber(proposalFee.amount).dp() <= feeTokenObj.precision,
          "invalid proposalFee token or precision"
        )
      )
        return;
    }
    const payTokenObj = await api.db.findOneInTable("tokens", "tokens", {
      symbol: payToken,
    });
    const voteTokenObj = await api.db.findOneInTable("tokens", "tokens", {
      symbol: voteToken,
    });
    if (
      !validateTokens(payTokenObj, voteTokenObj) ||
      !api.assert(
        api.BigNumber(maxAmountPerDay).dp() <= payTokenObj.precision,
        "maxAmountPerDay precision mismatch"
      ) ||
      !api.assert(
        api.BigNumber(voteThreshold).dp() <= voteTokenObj.precision,
        "voteThreshold precision mismatch"
      )
    )
      return;
    const now = new Date(`${api.steemBlockTimestamp}.000Z`);
...
actions.updateFund = async (payload) => {
  ...
  const authorizedUpdate =
    api.BigNumber(dtfUpdateFee).lte(0) || api.sender === api.owner
      ? true
      : utilityTokenBalance &&
        api.BigNumber(utilityTokenBalance.balance).gte(dtfUpdateFee);
  ...
  if (
    ...
    if (
      !api.assert(existingDtf, "DTF not found") ||
      !api.assert(
        existingDtf.creator === api.sender || api.owner === api.sender,
        "must be DTF creator"
      )
    )
      return;
    const payTokenObj = await api.db.findOneInTable("tokens", "tokens", {
      symbol: existingDtf.payToken,
    });
    const voteTokenObj = await api.db.findOneInTable("tokens", "tokens", {
      symbol: existingDtf.voteToken,
    });
    if (
      !api.assert(
        api.BigNumber(maxAmountPerDay).dp() <= payTokenObj.precision,
...
actions.setDtfActive = async (payload) => {
  const { fundId, active, isSignedWithActiveKey } = payload;
  ...
  if (
    api.assert(dtf, "DTF does not exist") &&
    api.assert(
      dtf.creator === api.sender || api.owner === api.sender,
      "must be DTF creator"
    )
  ) {
    dtf.active = !!active;
...
actions.createProposal = async (payload) => {
  ...
  if (dtf.proposalFee) {
    const feeTokenBalance = await api.db.findOneInTable("tokens", "balances", {
      account: api.sender,
      symbol: dtf.proposalFee.symbol,
    });
    authorizedCreation =
      api.BigNumber(dtf.proposalFee.amount).lte(0) || api.sender === api.owner
        ? true
        : feeTokenBalance &&
          api.BigNumber(feeTokenBalance.balance).gte(dtf.proposalFee.amount);
  }
  ...
  if (
    ...
    api.assert(
      ...
      "invalid title: between 1 and 80 characters"
    ) &&
    api.assert(
      typeof authorPermlink === "string" &&
        authorPermlink.length > 0 &&
        authorPermlink.length <= 255,
      "invalid authorPermlink: between 1 and 255 characters"
    ) &&
    api.assert(
      typeof amountPerDay === "string" &&
        api.BigNumber(amountPerDay).isInteger() &&
        api.BigNumber(amountPerDay).gt(0),
      "invalid amountPerDay: greater than 0"
    ) &&
    api.assert(
      api.BigNumber(amountPerDay).lte(dtf.maxAmountPerDay),
      "invalid amountPerDay: exceeds DTF maxAmountPerDay"
    ) &&
    api.assert(
      typeof payout === "object" &&
        typeof payout.type === "string" &&
        PayoutType.indexOf(payout.type) !== -1 &&
        (payout.type !== "contract" ||
          typeof payout.contractPayload === "object") &&
        typeof payout.name === "string" &&
        payout.name.length >= 3 &&
        payout.name.length <= 50,
      "invalid payout settings"
    ) &&
    ...
    if (api.sender !== api.owner && dtf.proposalFee) {
      if (dtf.proposalFee.method === "burn") {
        await api.executeSmartContract("tokens", "transfer", {
          to: "null",
          symbol: dtf.proposalFee.symbol,
          quantity: dtf.proposalFee.amount,
        });
...
actions.updateProposal = async (payload) => {
  ...
  if (
    !api.assert(
      typeof id === "string" && api.BigNumber(id).isInteger(),
      "invalid id"
    )
  )
    return;
  const proposal = await api.db.findOne("proposals", {
    _id: api.BigNumber(id).toNumber(),
  });
  if (
    !api.assert(proposal, "proposal does not exist") ||
    !api.assert(
      proposal.creator === api.sender || api.owner === api.sender,
      "must be proposal creator"
    )
  )
    return;
  const dtf = await api.db.findOne("funds", {
    id: proposal.fundId,
    active: true,
  });
  ...
  if (
    ...
    api.assert(
      typeof amountPerDay === "string" &&
        api.BigNumber(amountPerDay).isInteger() &&
        api.BigNumber(amountPerDay).gt(0) &&
        api.BigNumber(amountPerDay).lte(proposal.amountPerDay),
  ...
  if (
    ...
    if (
      ...
      for (let index = 0; index < approvals.length; index += 1) {
        ...
        const approvalProposal = await api.db.findOne("proposals", {
          _id: approval.to,
        });
      ...
      if (
        !api.assert(
          activeApprovals < params.maxAccountApprovals,
          `you can only approve ${params.maxAccountApprovals} active proposals`
        )
      )
        return;

      let approval = await api.db.findOne("approvals", {
        from: api.sender,
        to: proposal._id,
      });
...
actions.disapproveProposal = async (payload) => {
  ...
  if (
    ...
    if (
      ...
      const approval = await api.db.findOne("approvals", {
        from: api.sender,
        to: proposal._id,
      });
      if (
        api.assert(approval !== null, "you have not approved this proposal")
      ) {
        ...
        await api.db.update("accounts", acct);
        await updateProposalWeight(
          proposal._id,
          api.BigNumber(approvalWeight).negated()
        );
  ...
  if (acct !== null) {
    ...
    if (wIndex !== -1) {
      ...
      if (!api.BigNumber(deltaApprovalWeight).eq(0)) {
        ...
        for (let index = 0; index < approvals.length; index += 1) {
          const approval = approvals[index];
          const proposalPending = await updateProposalWeight(
            approval.to,
            deltaApprovalWeight,
            token
          );
  ...
  while (runningPay.gt(0)) {
    proposals = await api.db.find(
      "proposals",
      {
        ...
        active: true,
        approvalWeight: {
          $gt: { $numberDecimal: api.BigNumber(dtf.voteThreshold) },
        },
      ...
      offset,
      [
        { index: "byApprovalWeight", descending: true },
    ...
    for (let i = 0; i < proposals.length; i += 1) {
      if (
        api
          .BigNumber(proposals[i].amountPerDay)
          .times(tickPayRatio)
          .gte(runningPay)
      ) {
        proposals[i].tickPay = runningPay.toFixed(
          payTokenObj.precision,
  ...
  for (let i = 0; i < funded.length; i += 1) {
    ...
    if (fund.payout.type === "user") {
      await api.executeSmartContract("tokens", "issue", {
        to: fund.payout.name,
        symbol: payTokenObj.symbol,
        quantity: fund.tickPay,
      });
      ...
      await api.executeSmartContract(fund.payout.name, "receiveDtfTokens", {
        data: fund.payout.contractPayload,
        symbol: payTokenObj.symbol,
        quantity: fund.tickPay,
      });
    }
...
actions.checkPendingDtfs = async () => {
  if (api.assert(api.sender === "null", "not authorized")) {
    ...
    const tickTime = api
      ...
      .minus(params.dtfTickHours * 3600 * 1000)
      .toNumber();

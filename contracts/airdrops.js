...
const parseAirdrop = async (list, precision) => {
  ...
  for (let i = 0; i < list.length; i += 1) {
    ...
    if (
      api.assert(to, `list[${i}]: account name cannot be undefined`) &&
      api.assert(
        api.isValidAccountName(to),
        `list[${i}]: invalid account name`
      ) &&
      api.assert(quantity, `list[${i}]: quantity cannot be undefined`) &&
      api.assert(
        !api.BigNumber(quantity).isNaN(),
        `list[${i}]: invalid quantity`
      ) &&
      api.assert(
        api.BigNumber(quantity).gt(0),
        `list[${i}]: quantity must be positive`
      ) &&
      api.assert(
        api.BigNumber(quantity).dp() <= precision,
        `list[${i}]: quantity precision mismatch`
      )
    ) {
      airdrop.list.push({
        to,
  ...
  if (
    ...
    list.length === airdrop.list.length
  ) {
...
actions.newAirdrop = async (payload) => {
  const { symbol, type, list, isSignedWithActiveKey } = payload;
      ...
      if (
        ...
        api.assert(
          utilityToken &&
            ...
            api.BigNumber(utilityToken.balance).gte(airdrop.fee),
          "you must have enough tokens to cover the airdrop fee"
        ) &&
        api.assert(
          nativeToken &&
            nativeToken.balance &&
            api.BigNumber(nativeToken.balance).gte(airdrop.quantity),
          "you must have enough tokens to do the airdrop"
        )
      ) {
...
const processAirdrop = async (airdrop, maxTransactionsPerBlock) => {
  const { airdropId, list, symbol, type } = airdrop;

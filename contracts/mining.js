...
actions.createSSC = async () => {
  ...
  if (tableExists === false) {
    ...
    if (!params.updateIndex) {
      ...
      await api.db.addIndexes("miningPower", [
        { name: "byPoolIdAndAccount", index: { id: 1, account: 1 } },
      ]);
...
actions.updateParams = async (payload) => {
  ...
  if (poolUpdateFee) {
    if (
      !api.assert(
        typeof poolUpdateFee === "string" &&
          !api.BigNumber(poolUpdateFee).isNaN() &&
          api.BigNumber(poolUpdateFee).gte(0),
        "invalid poolUpdateFee"
      )
    )
      return;
  ...
  if (maxLotteriesPerBlock) {
    if (
      !api.assert(
        Number.isInteger(maxLotteriesPerBlock) && maxLotteriesPerBlock >= 1,
        "invalid maxLotteriesPerBlock"
      )
    )
      return;
  ...
  if (maxBalancesProcessedPerBlock) {
    if (
      !api.assert(
        Number.isInteger(maxBalancesProcessedPerBlock) &&
          maxBalancesProcessedPerBlock >= 1,
        "invalid maxBalancesProcessedPerBlock"
      )
    )
      return;
  ...
  if (processQueryLimit) {
    if (
      !api.assert(
        Number.isInteger(processQueryLimit) && processQueryLimit >= 1,
        "invalid processQueryLimit"
      )
    )
      return;
...
const findAndProcessAll = async (contractName, table, query, callback) => {
  ...
  let done = false;
  while (!done) {
    results = await api.db.findInTable(
      contractName,
      table,
      query,
      1000,
...
async function validateNftProperties(properties) {
  if (
    !api.assert(
      ...
      "invalid nftTokenMiner properties"
    )
  )
    return false;
  if (
  ...
  for (let i = 0; i < properties.length; i += 1) {
    ...
    for (let j = 0; j < propKeys.length; j += 1) {
      ...
      if (propKey === "op") {
        if (
          !api.assert(
            typeof prop.op === "string" && PROPERTY_OPS[prop.op],
        ...
        if (
          !api.assert(
            typeof prop.name === "string" && prop.name.length <= 16,
        ...
        if (
          !api.assert(
            typeof prop.burnChange === "object" &&
              typeof prop.burnChange.symbol === "string" &&
              api.BigNumber(prop.burnChange.quantity).isFinite() &&
              api.BigNumber(prop.burnChange.quantity).isPositive(),
            "nftTokenMiner properties burnChange invalid"
          )
        )
          return false;
        const token = await api.db.findOneInTable("tokens", "tokens", {
          symbol: prop.burnChange.symbol,
        });
        if (
          !api.assert(
            token,
...
function validateNftTypeMap(typeMap, properties) {
  if (
    !api.assert(
      typeMap && typeof typeMap === "object",
      "invalid nftTokenMiner typeMap"
    )
  )
    return false;
  ...
  for (let j = 0; j < types.length; j += 1) {
    ...
    if (
      !api.assert(
        Array.isArray(typeConfig) && typeConfig.length === properties.length,
        "nftTokenMiner typeConfig length mismatch"
      )
    )
      return false;
    for (let k = 0; k < typeConfig.length; k += 1) {
      ...
      if (
        !api.assert(
          !typeProperty.isNaN() && typeProperty.isFinite(),
          "nftTokenMiner typeConfig invalid"
        )
      )
        return false;
...
async function validateTokenMiners(tokenMiners, nftTokenMiner) {
  if (
    !api.assert(
      tokenMiners && Array.isArray(tokenMiners),
      "tokenMiners invalid"
    )
  )
    return false;
  ...
  for (let i = 0; i < tokenMiners.length; i += 1) {
    ...
    if (
      !api.assert(
        tokenMinerConfig &&
          tokenMinerConfig.symbol &&
          typeof tokenMinerConfig.symbol === "string",
        "tokenMiners invalid"
      )
    )
      return false;
    if (
      !api.assert(
        !tokenMinerSymbols.has(tokenMinerConfig.symbol),
        "tokenMiners cannot have duplicate symbols"
      )
    )
      return false;
    ...
    if (
      !api.assert(
        token && token.stakingEnabled,
        "tokenMiners must have staking enabled"
      )
    )
      return false;
    if (
      !api.assert(
        Number.isInteger(tokenMinerConfig.multiplier) &&
          tokenMinerConfig.multiplier >= 1 &&
          tokenMinerConfig.multiplier <= 100,
  ...
  if (nftTokenMiner) {
    if (
      !api.assert(
        nftTokenMiner.symbol && typeof nftTokenMiner.symbol === "string",
        "nftTokenMiner invalid"
      )
    )
      return false;
    const {
    ...
    if (
      !api.assert(
        nft && nft.delegationEnabled,
        "nftTokenMiner must have delegation enabled"
      )
    )
      return false;
    if (
      !api.assert(
        typeField && typeof typeField === "string",
    ...
    if (
      !api.assert(
        nft.properties[typeField] &&
          nft.properties[typeField].type === "string",
        "nftTokenMiner must have string type property"
      )
    )
      return false;
    if (equipField !== undefined) {
      if (
        !api.assert(
          equipField && typeof equipField === "string",
          "equipField must be a string"
        )
      )
        return false;
      if (
        !api.assert(
          nft.properties[equipField] &&
            nft.properties[equipField].type === "string",
          "nftTokenMiner must have string equip property"
        )
      )
        return false;
    }
    if (miningPowerField !== undefined) {
      if (
        !api.assert(
          miningPowerField && typeof miningPowerField === "string",
          "miningPowerField must be a string"
        )
      )
        return false;
      if (
        !api.assert(
          nft.properties[miningPowerField] &&
            nft.properties[miningPowerField].type === "string",
...
async function validateTokenMinersChange(
  ...
  if (
    !api.assert(
      tokenMiners.length === oldTokenMiners.length,
      "cannot change which tokens are in tokenMiners"
    )
  )
    return false;
  let changed = false;
  for (let i = 0; i < tokenMiners.length; i += 1) {
    ...
    if (
      !api.assert(
        oldConfig.symbol === newConfig.symbol,
        "cannot change which tokens are in tokenMiners"
      )
    )
      return false;
    if (
      !api.assert(
        Number.isInteger(newConfig.multiplier) &&
          newConfig.multiplier >= 1 &&
          newConfig.multiplier <= 100,
        "tokenMiner multiplier must be an integer from 1 to 100"
      )
    )
      return false;
  ...
  if (
    !api.assert(
      !!oldNftTokenMiner === !!nftTokenMiner,
      "cannot change nftTokenMiner token"
    )
  )
    return false;
  if (nftTokenMiner) {
    if (
      !api.assert(
        oldNftTokenMiner.symbol === nftTokenMiner.symbol,
        "cannot change nftTokenMiner token"
      )
    )
      return false;
    const { typeMap, properties, typeField, equipField, miningPowerField } =
      nftTokenMiner;
    if (
      !api.assert(
        typeField &&
          typeof typeField === "string" &&
          typeField === oldNftTokenMiner.typeField,
        "cannot change nftTokenMiner typeField"
      )
    )
      return false;
    if (oldNftTokenMiner.equipField) {
      if (
        !api.assert(
          equipField &&
            typeof equipField === "string" &&
            equipField === oldNftTokenMiner.equipField,
          "cannot change nftTokenMiner equipField"
        )
      )
        return false;
    } else if (
      !api.assert(!equipField, "cannot change nftTokenMiner equipField")
    )
      return false;
    if (oldNftTokenMiner.miningPowerField) {
      if (
        !api.assert(
          miningPowerField &&
            typeof miningPowerField === "string" &&
            miningPowerField === oldNftTokenMiner.miningPowerField,
          "cannot change nftTokenMiner miningPowerField"
        )
      )
        return false;
    } else if (
...
function computeMiningPower(miningPower, tokenMiners, nftTokenMiner) {
  ...
  for (let i = 0; i < tokenMiners.length; i += 1) {
    if (miningPower.balances[i]) {
      power = power.plus(
        api
          .BigNumber(miningPower.balances[i])
  ...
  if (nftTokenMiner && miningPower.nftBalances) {
    ...
    for (let i = 0; i < nftTokenMiner.properties.length; i += 1) {
      nftPower = nftPower
        .multipliedBy(miningPower.nftBalances[i])
        .dp(MAX_DIGITS);
    }
    ...
    if (miningPower.nftBalances[MINING_POWER_FIELD_INDEX]) {
      nftPower = nftPower.plus(
        miningPower.nftBalances[MINING_POWER_FIELD_INDEX]
      );
    }
...
async function updateMiningPower(
  ...
  let miningPower = await api.db.findOne("miningPower", {
    id: pool.id,
    account,
  });
  ...
  if (!miningPower) {
    ...
    oldMiningPower = computeMiningPower(
      miningPower,
      pool.tokenMiners,
      pool.nftTokenMiner
    );
    miningPower.balances[tokenIndex] = stake.plus(
      miningPower.balances[tokenIndex]
    );
  }
  const newMiningPower = computeMiningPower(
    miningPower,
    pool.tokenMiners,
    pool.nftTokenMiner
  );
 ...
 */
async function updateNftMiningPower(
  pool,
  nft,
  add,
  updatePoolTimestamp,
  ...
  if (miningPowerField) {
    extraNftMiningPower = sanitizeNftMiningPower(
      nft.properties[miningPowerField]
    );
  }
  if (!miningPower) {
    ...
    for (let i = 0; i < properties.length; i += 1) {
      ...
      if (add) {
        ...
        api.assert(
          false,
          "unexpected condition: remove without previous miningPower"
        );
        return api.BigNumber(0);
      }
    }
    ...
    if (miningPowerField) {
      ...
      if (add) {
        equippedNft.extraMiningPower = extraNftMiningPower;
        nftBalances[MINING_POWER_FIELD_INDEX] = api
          .BigNumber(nftBalances[MINING_POWER_FIELD_INDEX])
...
async function updateNftMiningPowerFromPropertyUpdate(
  pool,
  nft,
  accountOverride
) {
  ...
  let oldMiningPower = api.BigNumber(0);
  let extraNftMiningPower = api.BigNumber(0);
  const { typeField, equipField, miningPowerField } = pool.nftTokenMiner;
...
async function initMiningPower(
  pool,
  updatePoolTimestamp,
  params,
  token,
  ...
  while (!complete && offset < params.maxBalancesProcessedPerBlock) {
    balances = await api.db.findInTable(
      "tokens",
      "balances",
      { symbol: token, _id: { $gt: lastId } },
    ...
    for (let i = 0; i < balances.length; i += 1) {
      ...
      if (
        api.BigNumber(balance.stake).gt(0) ||
        api.BigNumber(balance.delegationsIn).gt(0)
      ) {
...
async function initNftMiningPower(
  pool,
  updatePoolTimestamp,
  params,
  nftTokenMiner,
  ...
  let complete = false;
  let nfts;
  const { symbol, equipField } = nftTokenMiner;
  while (!complete && offset < params.maxBalancesProcessedPerBlock) {
    const nftQuery = {
      _id: { $gt: lastId },
    };
...
async function resumePowerUpdate(pool, params) {
  let { inProgress, tokenIndex, nftTokenIndex, lastId } = pool.updating;
  ...
  if (
    tokenIndex === pool.tokenMiners.length &&
...
actions.setActive = async (payload) => {
  const { id, active, isSignedWithActiveKey } = payload;
  ...
  const minedTokenObject = await api.db.findOneInTable("tokens", "tokens", {
  ...
  if (
    !api.assert(
      minedTokenObject &&
        (minedTokenObject.issuer === api.sender ||
          (minedTokenObject.symbol === "'${CONSTANTS.UTILITY_TOKEN_SYMBOL}$'" &&
            api.sender === api.owner)),
      "must be issuer of minedToken"
    )
  ) {
    return;
  }
  ...
  const { nftTokenMiner } = pool;
  if (nftTokenMiner) {
    const nftTokenPool = await api.db.findOne("nftTokenPools", {
      symbol: nftTokenMiner.symbol,
      id: pool.id,
    });
    if (active && !nftTokenPool) {
      const otherNftTokenPools = await api.db.find("nftTokenPools", {
        symbol: nftTokenMiner.symbol,
      });
      if (
        !api.assert(
          !otherNftTokenPools || otherNftTokenPools.length < 2,
          "can have at most 2 active nft token pools for nft token"
        )
      ) {
        return;
      }
      await api.db.insert("nftTokenPools", {
        symbol: nftTokenMiner.symbol,
        id: pool.id,
      });
  ...
  pool.nextLotteryTimestamp = api
    .BigNumber(blockDate.getTime())
    .plus(pool.lotteryIntervalHours * 3600 * 1000)
    .toNumber();
  await api.db.update("pools", pool);
};
  ...
  if (
    ...
    api.assert(
      id &&
        typeof id === "string" &&
        lotteryAmount &&
        typeof lotteryAmount === "string" &&
        !api.BigNumber(lotteryAmount).isNaN() &&
        api.BigNumber(lotteryAmount).gt(0),
      "invalid params"
    )
  ) {
    if (
      api.assert(
        Number.isInteger(lotteryWinners) &&
          lotteryWinners >= 1 &&
          lotteryWinners <= 20,
        "invalid lotteryWinners: integer between 1 and 20 only"
      ) &&
      api.assert(
        Number.isInteger(lotteryIntervalHours) &&
      ...
      const pool = await api.db.findOne("pools", { id });
      if (api.assert(pool, "pool id not found")) {
        const minedTokenObject = await api.db.findOneInTable(
          "tokens",
          "tokens",
          { symbol: pool.minedToken }
        );
        ...
        if (
          api.assert(
            minedTokenObject &&
              (minedTokenObject.issuer === api.sender ||
                (minedTokenObject.symbol ===
                  "'${CONSTANTS.UTILITY_TOKEN_SYMBOL}$'" &&
                  api.sender === api.owner)),
            "must be issuer of minedToken"
          ) &&
          api.assert(
            api.BigNumber(lotteryAmount).dp() <= minedTokenObject.precision,
            "minedToken precision mismatch for lotteryAmount"
          )
        ) {
          if (!callingContractInfo) {
            ...
            if (validMinersChange) {
              ...
              if (validMinersChange.changed) {
                pool.updating.updatePoolTimestamp = api
                  .BigNumber(blockDate.getTime())
                  .toNumber();
              ...
              pool.nextLotteryTimestamp = api
                .BigNumber(blockDate.getTime())
                .plus(lotteryIntervalHours * 3600 * 1000)
                .toNumber();
            }
          } else {
            pool.lotteryWinners = lotteryWinners;
...
actions.changeNftProperty = async (payload) => {
  const { id, type, propertyName, changeAmount, isSignedWithActiveKey } =
    payload;
  ...
  if (
    !api.assert(
      id &&
        ...
        api.BigNumber(changeAmount).isFinite(),
      "invalid params"
    )
  )
    return;
  ...
  const propertyIndex = pool.nftTokenMiner.properties.findIndex(
    (p) => p.name === propertyName
  );
  const property = pool.nftTokenMiner.properties[propertyIndex];
  if (
    !api.assert(
      property && property.burnChange,
      "property not enabled for burn change"
    )
  )
    return;
  ...
  const fee = api
    .BigNumber(changeAmount)
    .abs()
    .multipliedBy(property.burnChange.quantity);

  if (
    !api.assert(
      fee.dp() <= token.precision,
      `fee precision mismatch for amount ${fee}`
    )
  )
    return;

  const balance = await api.db.findOneInTable("tokens", "balances", {
    account: api.sender,
    symbol: burnSymbol,
  });
  const authorized =
    api.BigNumber(fee).lte(0) ||
    (balance && api.BigNumber(balance.balance).gte(fee));

  if (
    !api.assert(
      authorized,
      `you must have enough tokens to cover the update fee of ${fee} ${burnSymbol}`
    )
  )
    return;

  typeProperties[propertyIndex] = api
    .BigNumber(typeProperties[propertyIndex])
    .plus(changeAmount);

  if (
    !validateNftTypeMap(
      pool.nftTokenMiner.typeMap,
      pool.nftTokenMiner.properties
    )
  )
    return;
...
function generatePoolId(pool) {
  const tokenMinerString =
    pool.externalContract && pool.externalMiners
      ? `EXT-${pool.externalMiners.replace(":", "")}`
      : pool.tokenMiners
          .map((t) => t.symbol.replace(".", "-"))
          .sort()
          .join(",");
  const nftTokenMinerString = pool.nftTokenMiner
    ? `:${pool.nftTokenMiner.symbol}`
    : "";
  return `${pool.minedToken.replace(
    ".",
    "-"
  )}:${tokenMinerString}${nftTokenMinerString}`;
}
...
actions.createPool = async (payload) => {
  ...
  if (externalMiners !== undefined) {
    if (
      !api.assert(
        typeof externalMiners === "string",
        "externalMiners must be a string"
      ) ||
      !api.assert(callingContractInfo, "must be called from a contract")
    )
      return;
  }
  ...
  if (
    ...
    if (
      api.assert(
        minedToken.length > 0 && minedToken.length <= 10,
      ...
      if (
        api.assert(minedTokenObject, "minedToken does not exist") &&
        // eslint-disable-next-line no-template-curly-in-string
        api.assert(
          minedTokenObject.issuer === api.sender ||
            (minedTokenObject.symbol ===
        ...
        if (
          ...
          if (callingContractInfo) {
            if (
              !api.assert(
                !nftTokenMiner,
                "external nftTokenMiner not currently supported"
              )
            )
              return;
          ...
          const existingPool = await api.db.findOne("pools", {
            id: newPool.id,
          });
          if (api.assert(!existingPool, "pool already exists")) {
            if (tokenMiners) {
              for (let i = 0; i < tokenMiners.length; i += 1) {
                ...
                await api.db.insert("tokenPools", {
            ...
            if (
              api.sender !== api.owner &&
              api.BigNumber(poolCreationFee).gt(0)
            ) {
              await api.executeSmartContract("tokens", "transfer", {
                // eslint-disable-next-line no-template-curly-in-string
                to: "null",
...
async function runLottery(pool, params) {
  ...
  const winningAmount = api
    .BigNumber(pool.lotteryAmount)
    .dividedBy(pool.lotteryWinners)
  ...
  if (!pool.externalContract) {
    for (let i = 0; i < pool.lotteryWinners; i += 1) {
      winningNumbers[i] = api
        .BigNumber(pool.totalPower)
        .multipliedBy(api.random());
    }
  } else if (pool.externalContract === "marketpools") {
    const marketpool = await api.db.findOneInTable("marketpools", "pools", {
      tokenPair: pool.externalMiners,
    });
    for (let i = 0; i < pool.lotteryWinners; i += 1) {
      winningNumbers[i] = api
        .BigNumber(marketpool.totalShares)
        .multipliedBy(api.random());
    }
  }
  let offset = 0;
  ...
  while (computedWinners < pool.lotteryWinners) {
    if (!pool.externalContract) {
      ...
      for (let i = 0; i < miningPowers.length; i += 1) {
        miningPowers[i].power = {
          $numberDecimal: api
            .BigNumber(miningPowers[i].shares)
            .toFixed(minedToken.precision, api.BigNumber.ROUND_HALF_UP),
        };
      }
    }
    for (let i = 0; i < miningPowers.length; i += 1) {
      ...
      nextCumulativePower = cumulativePower.plus(
        miningPower.power.$numberDecimal
      );
      for (let j = 0; j < pool.lotteryWinners; j += 1) {
        const currentWinningNumber = winningNumbers[j];
        if (
          cumulativePower.lte(currentWinningNumber) &&
    ...
    if (
      computedWinners === pool.lotteryWinners ||
      miningPowers.length < params.processQueryLimit
    ) {
      break;
    }
    offset += params.processQueryLimit;
  ...
  for (let i = 0; i < winners.length; i += 1) {
    ...
    await api.executeSmartContract("tokens", "issue", {
      to: winner.winner,
      symbol: minedToken.symbol,
      quantity: winningAmount,
    });
  }
  // eslint-disable-next-line no-param-reassign
  pool.nextLotteryTimestamp = api
...
actions.handleStakeChange = async (payload) => {
  const { account, symbol, quantity, delegated, callingContractInfo } = payload;
  if (
    api.assert(
    ...
    await findAndProcessAll(
      ...
      async (tokenPool) => {
        ...
        let adjusted;
        if (delegated) {
          adjusted = await updateMiningPower(
            pool,
            symbol,
            account,
            0,
...
actions.handleNftDelegationChange = async (payload) => {
  const { symbol, nft, add, callingContractInfo } = payload;
  if (
    api.assert(
    ...
    await findAndProcessAll(
      ...
      async (tokenPool) => {
        ...
        const { equipField, typeField, typeMap, miningPowerField } =
          pool.nftTokenMiner;
        if (equipField) {
          return;
        }

        const typeProperties = typeMap[nft.properties[typeField]];
...
actions.handleNftSetProperty = async (payload) => {
  const { symbol, nft, propertyName, oldValue, callingContractInfo } = payload;
  if (
    api.assert(
      callingContractInfo && callingContractInfo.name === "nft",
      "must be called from nft contract"
    ...
    await findAndProcessAll(
      ...
      async (tokenPool) => {
        ...
        if (equipField && propertyName === equipField) {
          ...
          if (
            !equipField ||
            api.isValidAccountName(nft.properties[equipField])
          ) {

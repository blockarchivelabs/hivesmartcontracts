...
actions.createSSC = async () => {
  ...
  if (tableExists === false) {
    ...
    if (!params.updateIndex) {
      await api.db.addIndexes("batches", [
        { name: "lastTickTime", index: { lastTickTime: 1 } },
      ]);
      ...
      for (let i = 0; i < syncDists.length; i += 1) {
        ...
        if (dist.tokenBalances) {
          let update = false;
          const hiveIndex = dist.tokenBalances.findIndex(
            (t) => t.symbol === "STEEMP"
          );
          if (hiveIndex !== -1) {
            dist.tokenBalances[hiveIndex].quantity = api
              .BigNumber("0")
              .toFixed(8);
            update = true;
          }
          const simIndex = dist.tokenBalances.findIndex(
            (t) => t.symbol === "SIM"
          );
          if (simIndex !== -1) {
            dist.tokenBalances[simIndex].quantity = api
              .BigNumber("0")
              .toFixed(3);
            update = true;
          }
...
actions.updateParams = async (payload) => {
  ...
  if (distCreationFee) {
    if (
      !api.assert(
        typeof distCreationFee === "string" &&
          !api.BigNumber(distCreationFee).isNaN() &&
          api.BigNumber(distCreationFee).gte(0),
        "invalid distCreationFee"
      )
    )
      return;
  ...
  if (distUpdateFee) {
    if (
      !api.assert(
        typeof distUpdateFee === "string" &&
          !api.BigNumber(distUpdateFee).isNaN() &&
          api.BigNumber(distUpdateFee).gte(0),
        "invalid distUpdateFee"
      )
    )
      return;
  ...
  if (distTickHours) {
    if (
      !api.assert(
        typeof distTickHours === "string" &&
          api.BigNumber(distTickHours).isInteger() &&
          api.BigNumber(distTickHours).gte(1),
        "invalid distTickHours"
      )
    )
      return;
  ...
  if (maxDistributionsLimit) {
    if (
      !api.assert(
        typeof maxDistributionsLimit === "string" &&
          api.BigNumber(maxDistributionsLimit).isInteger() &&
          api.BigNumber(maxDistributionsLimit).gte(1),
        "invalid maxDistributionsLimit"
      )
    )
      return;
    params.maxDistributionsLimit = api
      .BigNumber(maxDistributionsLimit)
      .toNumber();
  }
  if (maxTransferLimit) {
    if (
      !api.assert(
        typeof maxTransferLimit === "string" &&
          api.BigNumber(maxTransferLimit).isInteger() &&
          api.BigNumber(maxTransferLimit).gte(1),
        "invalid maxTransferLimit"
      )
    )
      return;
  ...
  if (processQueryLimit) {
    if (
      !api.assert(
        typeof processQueryLimit === "string" &&
          api.BigNumber(processQueryLimit).isInteger() &&
          api.BigNumber(processQueryLimit).gte(1),
...
async function validateMinPayout(tokenMinPayout) {
  if (
    !api.assert(
      tokenMinPayout && Array.isArray(tokenMinPayout),
      "tokenMinPayout must be an array"
    )
  )
    return false;
  if (
    !api.assert(
      tokenMinPayout.length >= 1,
      "specify at least one minimum payout configuration"
    )
  )
    return false;
  ...
  for (let i = 0; i < tokenMinPayout.length; i += 1) {
    ...
    if (
      !api.assert(
        tokenMinPayoutConfig &&
          tokenMinPayoutConfig.symbol &&
          typeof tokenMinPayoutConfig.symbol === "string",
        "tokenMinPayout invalid"
      )
    )
      return false;
    ...
    if (
      !api.assert(
        !tokenMinPayoutSymbols.has(tokenMinPayoutConfig.symbol),
        "tokenMinPayout cannot have duplicate symbols"
      )
    )
      return false;
    ...
    if (
      !api.assert(
        tokenMinPayoutConfig.quantity &&
          api.BigNumber(tokenMinPayoutConfig.quantity).dp() <= 3 &&
          api.BigNumber(tokenMinPayoutConfig.quantity).gte(0),
        "invalid quantity"
      )
    )
      return false;
...
async function validateRecipients(params, tokenRecipients) {
  if (
    !api.assert(
      tokenRecipients && Array.isArray(tokenRecipients),
      "tokenRecipients must be an array"
    )
  )
    return false;
  if (
    !api.assert(
      tokenRecipients.length >= 1 &&
        tokenRecipients.length <= params.maxTransferLimit,
      `1-${params.maxTransferLimit} tokenRecipients are supported`
    )
  )
    return false;
  ...
  for (let i = 0; i < tokenRecipients.length; i += 1) {
    ...
    if (
      !api.assert(
        tokenRecipientsConfig &&
          tokenRecipientsConfig.account &&
          typeof tokenRecipientsConfig.account === "string",
        "tokenRecipients invalid"
      ) &&
      !api.assert(
        tokenRecipientsConfig.account.length >= 3 &&
          tokenRecipientsConfig.account.length <= 16,
        "invalid account"
      )
    )
      return false;
    ...
    if (
      !api.assert(
        !tokenRecipientsAccounts.has(tokenRecipientsConfig.account),
        "tokenRecipients cannot have duplicate accounts"
      )
    )
      return false;
    ...
    if (
      !api.assert(
        Number.isInteger(tokenRecipientsConfig.pct) &&
          tokenRecipientsConfig.pct >= 1 &&
          tokenRecipientsConfig.pct <= 100,
    ...
    if (
      !api.assert(
        ["user", "contract"].includes(tokenRecipientsConfig.type),
        "tokenRecipients type must be user or contract"
      )
    )
      return false;
    if (
      tokenRecipientsConfig.type === "contract" &&
      !api.assert(
        typeof tokenRecipientsConfig.contractPayload === "object",
        "contract recipients must have contractPayload"
      )
    )
      return false;
  }
  if (
    !api.assert(
      tokenRecipientsTotalShare === 100,
      "tokenRecipients pct must total 100"
    )
  )
    return false;
  return true;
    ...
    if (
      !api.assert(
        typeof obj.periodBonusPct === "string" &&
          ...
          api.BigNumber(obj.periodBonusPct).gt(0) &&
          api.BigNumber(obj.periodBonusPct).lt(100) &&
          typeof obj.numPeriods === "string" &&
          api.BigNumber(obj.numPeriods).isInteger() &&
          api.BigNumber(obj.numPeriods).gt(0) &&
          api.BigNumber(obj.numPeriods).lt(5555),
        "invalid bonusCurve settings"
      )
    )
      return false;
  } else if (
    !api.assert(Object.keys(obj).length === 0, "invalid bonusCurve settings")
  )
    return false;
  return true;
}
...
async function validatePool(tokenPair) {
  return (
    (await api.db.findOneInTable("marketpools", "pools", { tokenPair })) !==
    null
  );
}
...
actions.create = async (payload) => {
  ...
  const authorizedCreation =
    api.BigNumber(distCreationFee).lte(0) || api.sender === api.owner
      ? true
      : utilityTokenBalance &&
        api.BigNumber(utilityTokenBalance.balance).gte(distCreationFee);
  ...
  if (
    ...
    api.assert(
      typeof strategy === "string" && DistStrategy.indexOf(strategy) !== -1,
      "invalid strategy"
    ) &&
    api.assert(
      typeof numTicks === "string" &&
        api.BigNumber(numTicks).isInteger() &&
        api.BigNumber(numTicks).gt(0) &&
        api.BigNumber(numTicks).lte(5555),
      "numTicks must be a number between 1 and 5555"
    )
  ) {
    ...
    if (
      strategy === "fixed" &&
      (await validateMinPayout(tokenMinPayout)) &&
      (await validateRecipients(params, tokenRecipients))
    ) {
      newDist.tokenMinPayout = tokenMinPayout;
      newDist.tokenRecipients = tokenRecipients;
    } else if (strategy === "pool") {
      if (!api.assert(await validatePool(tokenPair), "invalid tokenPair"))
        return;
      if (
        excludeAccount !== undefined &&
        !api.assert(
          Array.isArray(excludeAccount),
          "excludeAccount must be an array"
        )
      )
        return;
...
actions.update = async (payload) => {
  ...
  const authorizedCreation =
    api.BigNumber(distUpdateFee).lte(0) || api.sender === api.owner
      ? true
      : utilityTokenBalance &&
        api.BigNumber(utilityTokenBalance.balance).gte(distUpdateFee);
  ...
  if (
    ...
    if (
      api.assert(exDist, "distribution not found") &&
      api.assert(
        api.sender === api.owner || api.sender === exDist.creator,
      ...
      if (exDist.strategy === "fixed") {
        ...
        if (
          tokenPair !== undefined &&
          api.assert(await validatePool(tokenPair), "invalid tokenPair")
        ) {
...
actions.setActive = async (payload) => {
  ...
  if (
    api.assert(dist, "distribution id not found") &&
    api.assert(
      dist.creator === api.sender || api.owner === api.sender,
      "you must be the creator of this distribution"
    )
  ) {
    dist.active = !!active;
...
async function updateTokenBalances(dist, token, quantity) {
  ...
  if (upDist.tokenBalances) {
    const tIndex = upDist.tokenBalances.findIndex(
      (t) => t.symbol === token.symbol
    );
    if (tIndex === -1) {
      upDist.tokenBalances.push({ symbol: token.symbol, quantity });
    } else {
      upDist.tokenBalances[tIndex].quantity = api
        .BigNumber(upDist.tokenBalances[tIndex].quantity)
        .plus(quantity)
        .toFixed(token.precision, api.BigNumber.ROUND_DOWN);
    }
  } else {
    upDist.tokenBalances = [{ symbol: token.symbol, quantity }];
  }
  await api.db.update("batches", upDist);
}

actions.deposit = async (payload) => {
  const { id, symbol, quantity, isSignedWithActiveKey } = payload;
  ...
  if (
    ...
    !api.assert(
      typeof quantity === "string" && api.BigNumber(quantity).gt(0),
      "invalid quantity"
    ) ||
    !api.assert(
      api.BigNumber(quantity).dp() <= depToken.precision,
  ...
  if (
    api.assert(dist, "distribution id not found") &&
    api.assert(dist.active, "distribution must be active to deposit")
  ) {
    if (
      dist.strategy === "fixed" &&
      !api.assert(
        validateIncomingToken(dist, symbol),
        `${symbol} is not accepted by this distribution`
      )
    )
      return;
...
actions.receiveDtfTokens = async (payload) => {
  const { data, symbol, quantity, callingContractInfo } = payload;

  if (
    !api.assert(
      callingContractInfo && callingContractInfo.name === "tokenfunds",
      "not authorized"
    )
  )
    return;
  if (
    !api.assert(
      typeof data === "object" &&
        ...
        api.BigNumber(data.distId).isInteger(),
      "invalid DTF payload"
    )
  )
    return;

  const dist = await api.db.findOne("batches", {
    _id: api.BigNumber(data.distId).toNumber(),
  });
  if (
    api.assert(dist, "distribution id not found") &&
    api.assert(dist.active, "distribution must be active to deposit")
    ...
    if (
      ...
      return;
    const depToken = await api.db.findOneInTable("tokens", "tokens", {
      symbol,
    });
    await updateTokenBalances(dist, depToken, quantity);
...
async function getEffectiveShares(params, dist, lp) {
  ...
  if (typeof dist.bonusCurve.numPeriods === "string") {
    if (
      timeDiff.lt(
        params.distTickHours * dist.bonusCurve.numPeriods * 3600 * 1000
      )
    ) {
      multiplier = api
        .BigNumber(dist.bonusCurve.periodBonusPct)
        .dividedBy("100")
        .times(
          timeDiff
            .dividedBy(params.distTickHours * 3600 * 1000)
...
async function getPoolRecipients(params, dist) {
  ...
  let offset = 0;
  let processQuery = [];
  const pool = await api.db.findOneInTable("marketpools", "pools", {
...
async function payRecipient(
  ...
  if (api.BigNumber(quantity).gt(0)) {
    ...
    if (res.errors) {
      api.debug(
        `Error paying out distribution of ${quantity} ${symbol} to ${account} (TXID ${api.transactionId}): \n${res.errors}`
      );
      return false;
    }
    return true;
...
async function runDistribution(dist, params, flush = false) {
  ...
  const payTokens = dist.tokenBalances.filter((d) =>
    api.BigNumber(d.quantity).gt(0)
  );
    ...
    if (dist.strategy === "fixed") {
      ...
      while (tokenRecipients.length > 0) {
        ...
        for (let i = 0; i < payTokens.length; i += 1) {
          const payToken = await api.db.findOneInTable("tokens", "tokens", {
            symbol: payTokens[i].symbol,
          });
          const minPayout = dist.tokenMinPayout.find(
            (p) => p.symbol === payTokens[i].symbol
          );
          if (
            api.BigNumber(payTokens[i].quantity).gt(minPayout.quantity) ||
            flush
          ) {
            const payoutShare = api
              .BigNumber(payTokens[i].quantity)
              .multipliedBy(tr.pct)
              .dividedBy(100)
            ...
            if (api.BigNumber(payoutShare).gt(0)) {
              if (
                ...
                const tbIndex = upDist.tokenBalances.findIndex(
                  (b) => b.symbol === payTokens[i].symbol
                );
                upDist.tokenBalances[tbIndex].quantity = api
                  .BigNumber(upDist.tokenBalances[tbIndex].quantity)
                  .minus(payoutShare)
                  .toFixed(payToken.precision, api.BigNumber.ROUND_DOWN);
                api.emit("payment", {
                  distId: dist._id,
      ...
      const shareTotal = tokenRecipients.reduce(
        (acc, cur) => acc.plus(cur.effShares),
        api.BigNumber(0)
      );
      if (
        api.assert(shareTotal.gt(0), "no liquidity shares for this tokenPair")
      ) {
        let payTxCount = 0;
        const storePending = [];
        while (tokenRecipients.length > 0) {
        ...
        if (storePending.length > 0)
          await api.db.insert("pendingPayments", {
            dueTime: blockDate.getTime(),
            accounts: storePending,
          });
      }
    }
  }
...
actions.flush = async (payload) => {
  ...
  if (
    ...
    api.assert(
      api.sender === api.owner || api.sender === dist.creator,
      "must be owner or creator"
    )
  ) {
    const params = await api.db.findOne("params", {});
...
async function runPendingPay(pendingPay, params) {
  const payLimit = Math.min(
    params.maxTransferLimit,
    pendingPay.accounts.length
  );
...
actions.checkPendingDistributions = async () => {
  if (api.assert(api.sender === "null", "not authorized")) {
    ...
    const pendingDists = await api.db.find(
      ...
      0,
      [
        { index: "lastTickTime", descending: false },
        { index: "_id", descending: false },
      ]
    );
    ...
    if (pendingDists.length > 0) {
      ...
      const pendingPay = await api.db.find(
        ...
        0,
        [
          { index: "dueTime", descending: false },
          { index: "_id", descending: false },
        ]
      );

...
const getOrderBook = async (table, symbol, descending) => {
  const orders = await api.db.findInTable("market", table, { symbol }, 0, 0, [
...
const cancelOrders = async (orders, type) => {
  for (let k = 0; k < orders.length; k += 1) {
    ...
    await api.executeSmartContract("market", "cancel", {
...
const countBalanceInBuyOrders = (orders) =>
  orders.reduce(
    (t, v) => t.plus(api.BigNumber(v.tokensLocked)),
    api.BigNumber(0)
  );
const countBalanceInSellOrders = (orders) =>
  orders.reduce((t, v) => t.plus(api.BigNumber(v.quantity)), api.BigNumber(0));
...
const getOrderData = (
  ...
  data.isTopMine =
    data.topOrder && data.topOrder.account === account ? true : false;
  data.topPrice = data.topOrder
    ? api.BigNumber(data.topOrder.price)
    : api.BigNumber(0);
  ...
  while (counter < orders.length) {
    ...
    if (
      !nextTopPrice.eq(data.topPrice) &&
      qtyLimit.lt(orders[counter].quantity)
    ) {
  ...
  data.myTopPrice =
    myOrders.length > 0 ? api.BigNumber(myOrders[0].price) : api.BigNumber(0);
  data.numOrdersAtMyPrice = 0;
  if (data.myTopPrice.gt(0)) {
    data.numOrdersAtMyPrice = orders.reduce(
      (t, v) => (api.BigNumber(v.price).eq(data.myTopPrice) ? t + 1 : t),
      0
    );
...
const tickMarket = async (market, txIdPrefix) => {
  ...
  if (DEBUG_MODE) {
    api.debug(
      `ticking market for user: ${market.account}, symbol: ${market.symbol}`
    );
  }
  ...
  const isBuyBookEmpty =
    buyOrders.length === 0 || buyOrders.length === myBuyOrders.length;
  const isSellBookEmpty =
    sellOrders.length === 0 || sellOrders.length === mySellOrders.length;
  ...
  const isMaxBuyDistExceeded =
    bb.topPrice.gt(bb.nextTopPrice.plus(maxDistFromNext)) || isBuyBookEmpty;
  ...
  const isMaxSellDistExceeded =
    sb.topPrice.lt(sb.nextTopPrice.minus(maxDistFromNext)) || isSellBookEmpty;
  ...
  if (
    ...
    ((bb.myTopPrice.lt(bb.topPrice) && bb.topPrice.lt(maxBidPrice)) ||
      ...
      (bb.numOrdersAtMyPrice > 1 &&
        !bb.isTopMine &&
        bb.myTopPrice.lt(newTopBuyPrice)))
  ) {
  ...
  if (
    ...
    ((sb.myTopPrice.gt(sb.topPrice) && sb.topPrice.gt(minSellPrice)) ||
      ...
      (sb.numOrdersAtMyPrice > 1 &&
        !sb.isTopMine &&
        sb.myTopPrice.gt(newTopSellPrice)))
  ) {
  ...
  let orderCount = 0;
  if (
    (myBuyOrders.length === 0 || shouldReplaceBuyOrder) &&
    maxBaseToSpend.gte(minBaseToSpend)
  ) {
    const tokensToBuy = getClosestAmount(
      maxBaseToSpend,
      newTopBuyPrice,
      market.precision
    );
  ...
  if (
    (mySellOrders.length === 0 || shouldReplaceSellOrder) &&
    maxTokensToSell.gte(minTokensToSell) &&
    sellOrders.length > 0
  ) {
    if (DEBUG_MODE) {
...
actions.tick = async (payload) => {
  ...
  const { markets, txIdBase } = payload;

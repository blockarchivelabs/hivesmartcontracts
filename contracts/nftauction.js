...
actions.createSSC = async () => {
  ...
  if (tableExists === false) {
    await api.db.createTable("auctions", [
      "auctionId",
      "symbol",
      "lastValidLead",
      "expiryTimestamp",
...
actions.updateParams = async (payload) => {
  if (api.assert(api.sender === api.owner, "not authorized")) {
      ...
      if (
        !api.assert(
          typeof creationFee === "string" &&
            api.BigNumber(creationFee).isFinite() &&
    ...
    if (minBidIncrementPercent) {
      if (
        !api.assert(
          Number.isInteger(minBidIncrementPercent) &&
            minBidIncrementPercent > 0 &&
            minBidIncrementPercent <= MAX_BID_INCREMENT_PERCENT,
          "invalid minBidIncrementPercent"
        )
      )
        return;
    ...
    if (cancelLockTimeMillis) {
      if (
        !api.assert(
          Number.isInteger(cancelLockTimeMillis) && cancelLockTimeMillis > 0,
          "invalid cancelLockTimeMillis"
        )
      )
        return;
    ...
    if (expiryTimeMillis) {
      if (
        !api.assert(
          Number.isInteger(expiryTimeMillis) && expiryTimeMillis > 0,
          "invalid expiryTimeMillis"
        )
      )
        return;
    ...
    if (maxExpiryTimeMillis) {
      if (
        !api.assert(
          Number.isInteger(maxExpiryTimeMillis) && maxExpiryTimeMillis > 0,
          "invalid maxExpiryTimeMillis"
        )
      )
        return;
    ...
    if (auctionsPerBlock) {
      if (
        !api.assert(
          Number.isInteger(auctionsPerBlock) && auctionsPerBlock > 0,
          "invalid auctionsPerBlock"
        )
      )
        return;
...
const cancelAuction = async (auction) => {
  const { auctionId, symbol, seller, nftIds, priceSymbol, bids } = auction;
...
const settleAuction = async (auction, index = null) => {
  const { auctionId, symbol, seller, nftIds, priceSymbol, bids, currentLead } =
    auction;
  ...
  if (bids.length > 0) {
    ...
    if (
      marketParams &&
      marketParams.officialMarket &&
      marketParams.agentFeePercent !== undefined &&
      marketParams.agentFeePercent > 0 &&
      fee.gt(0)
    ) {
    ...
    const officialMarketAccount =
      marketParams && marketParams.officialMarket
        ? marketParams.officialMarket
        : leadBid.marketAccount;
  ...
  if (
    api.assert(
      nfts &&
        ...
        Array.isArray(nfts) &&
        minBid &&
        typeof minBid === "string" &&
        api.BigNumber(minBid).isFinite() &&
        finalPrice &&
        typeof finalPrice === "string" &&
        api.BigNumber(finalPrice).isFinite() &&
        priceSymbol &&
    ...
    api.assert(
      ...
      `cannot process more than ${MAX_NUM_UNITS_OPERABLE} NFT instances at once`
    )
  ) {
    const marketParams = await api.db.findOne("marketParams", { symbol });
    if (marketParams && marketParams.minFeePercent !== undefined) {
      if (
        !api.assert(
          feePercent >= marketParams.minFeePercent,
          `feePercent must be >= ${marketParams.minFeePercent}`
        )
      ) {
        return;
      }
    ...
    const maxExpiryTimestamp = api
      .BigNumber(timestamp)
      .plus(params.maxExpiryTimeMillis)
      .toNumber();

    if (
      api.assert(token, "priceSymbol does not exist") &&
      // minBid checks
      api.assert(
        api.BigNumber(minBid).gt(0) && countDecimals(minBid) <= token.precision,
        "invalid minBid"
      ) &&
      // finalPrice checks
      api.assert(
        api.BigNumber(finalPrice).gt(0) &&
          countDecimals(finalPrice) <= token.precision,
        "invalid finalPrice"
      ) &&
      // expiry checks
      api.assert(
      ...
      if (
        api.assert(
          utilityToken &&
            ...
            api.BigNumber(utilityToken.balance).gte(params.creationFee),
        ...
        if (
          api.assert(
            nftTransfer.events,
            "failed to trasfer NFTs to the contract"
          )
        ) {
          const nftIds = [];
...
actions.settle = async (payload) => {
  const { auctionId, account, isSignedWithActiveKey } = payload;
  ...
  if (
    ...
    if (
      api.assert(auction, "auction does not exist or has been expired") &&
      api.assert(
        auction.seller === api.sender,
        "you must be the owner of the auction"
      )
    ) {
      if (
        api.assert(auction.bids.length > 0, "there are no bids in the auction")
      ) {
        ...
        if (account) {
          ...
          if (
            !api.assert(
              auction.bids[id],
              "no bid from account found in the auction"
            )
          )
            return;
        }
...
actions.cancel = async (payload) => {
  const { auctionId, isSignedWithActiveKey } = payload;
...
actions.bid = async (payload) => {
  const { auctionId, bid, marketAccount, isSignedWithActiveKey } = payload;
  ...
  if (
    ...
    if (api.assert(auction, "auction does not exist or has been expired")) {
      ...
      if (
        api.assert(api.sender !== seller, "auction seller can not bid") &&
        api.assert(
          api.BigNumber(bid).gt(0) && countDecimals(bid) <= token.precision,
          "invalid bid"
        ) &&
        api.assert(
          api.BigNumber(bid).gte(minBid),
          `bid can not be less than ${minBid}`
        ) &&
        api.assert(expiryTimestamp >= timestamp, "auction has been expired") &&
        api.assert(
          api.isValidAccountName(marketAccount),
          "invalid marketAccount"
        )
      ) {
        let nbTokensToLock = api.BigNumber(bid).gt(finalPrice)
          ? finalPrice
          : bid;
        // find if the account has any previous bid in this auction
        const previousBidIndex = auction.bids.findIndex(
        ...
        if (previousBid) {
          if (
            !api.assert(
              api.BigNumber(nbTokensToLock).gt(previousBid.bid),
              "bid must be greater than your previous bid"
            )
          )
            return;
        ...
        const priceToken = await api.db.findOneInTable("tokens", "balances", {
          account: api.sender,
          symbol: priceSymbol,
        });
        ...
        if (
          api.assert(
            priceToken &&
              priceToken.balance &&
              api.BigNumber(priceToken.balance).gte(nbTokensToLock),
          ...
          if (
            ...
            const newBidIndex = auction.bids.findIndex(
              (el) => el.account === api.sender
            );
            ...
            if (currentLead !== null) {
              ...
              const minBidIncrementPct =
                params.minBidIncrementPercent / MAX_BID_INCREMENT_PERCENT;
              // quantity increment the new bid requires to take the lead
              const minBidIncrement = api
                .BigNumber(leadBid.bid)
                .multipliedBy(minBidIncrementPct)
                .toFixed(token.precision);

              if (
                api
                  .BigNumber(newBid.bid)
                  .minus(leadBid.bid)
                  .gte(minBidIncrement)
              ) {
...
actions.cancelBid = async (payload) => {
  ...
  if (
    ...
    if (api.assert(auction, "auction does not exist or has been expired")) {
      const { priceSymbol, currentLead, expiryTimestamp, lastLeadUpdate } =
        auction;
      ...
      if (
        api.assert(bid, "you do not have a bid in this auction") &&
        api.assert(expiryTimestamp >= timestamp, "auction has been expired")
      ) {
        const params = await api.db.findOne("params", {});
        const timeRemaining = api
          .BigNumber(lastLeadUpdate)
          .plus(params.expiryTimeMillis)
          .minus(timestamp);
        const timeRemainingExpire = api
        ...
        if (
          api.assert(
            timeRemaining.gt(params.cancelLockTimeMillis) &&
              timeRemainingExpire.gt(params.cancelLockTimeMillis),
            "can not cancel bid when auction is about to settle"
          )
        ) {
    ...
    api.assert(
      (officialMarket === undefined ||
        (officialMarket &&
          ...
          api.isValidAccountName(officialMarket))) &&
        (agentFeePercent === undefined ||
          (typeof agentFeePercent === "number" &&
            agentFeePercent >= 0 &&
            agentFeePercent <= 10000 &&
            Number.isInteger(agentFeePercent))) &&
        (minFeePercent === undefined ||
          (typeof minFeePercent === "number" &&
            minFeePercent >= 0 &&
            minFeePercent <= 10000 &&
            Number.isInteger(minFeePercent))),
      "invalid params"
    )
  ) {

...
actions.roll = async (payload) => {
  ...
  if (
    api.assert(
      roll && Number.isInteger(roll) && roll >= 2 && roll <= 96,
      "roll must be an integer and must be between 2 and 96"
    ) &&
    api.assert(
      amount &&
        typeof amount === "string" &&
        api.BigNumber(amount).dp() <= 3 &&
    ...
    if (
      api.assert(
        api.BigNumber(amount).gte(params.minBet) &&
          api.BigNumber(amount).lte(params.maxBet),
        "amount must be between minBet and maxBet"
      )
    ) {
      ...
      const res = await api.executeSmartContract(
        "tokens",
        "transferToContract",
        { symbol: STEEM_PEGGED_SYMBOL, quantity: amount, to: CONTRACT_NAME }
      );
      ...
      if (
        ...
        if (randomRoll < roll) {
          ...
          // emit an event
          api.emit("results", {
            memo: `you won. roll: ${randomRoll}, your bet: ${roll}`,
          });
        } else {
          // emit an event
          api.emit("results", {
            memo: `you lost. roll: ${randomRoll}, your bet: ${roll}`,
          });
        }
      }
      // else,

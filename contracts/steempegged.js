...
actions.buy = async (payload) => {
  ...
  if (recipient && amountSTEEMSBD && isSignedWithActiveKey) {
    ...
    // HIVE
    if (api.assert(unit === "STEEM, "only HIVE can be used")) {
  ...
  if (
    api.assert(
      quantity &&
        ...
        api.BigNumber(quantity).dp() <= 3,
      "invalid params"
    ) &&
    api.assert(
      api.BigNumber(quantity).gte(0.002),
      ...
      if (
        ...
        res.events.find(
          (el) =>
            ...
            el.data.to === api.owner &&
            el.data.quantity === quantity &&
            el.data.symbol === "STEEMP"
        ) !== undefined
      ) {
        // withdrawal

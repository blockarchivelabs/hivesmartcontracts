...
const initialTickContracts = new Set(
  CONSTANTS.INITIAL_CONTRACT_TICKS.map((t) => t.contract)
);
...
describe("ticks", function () {
  ...
  it("should trigger ticks at expected refblock", (done) => {
    new Promise(async (resolve) => {
      ...
      initialTickContracts.forEach((c) =>
        transactions.push(
          new Transaction(
            ...
            "update",
            JSON.stringify(
              setupContractPayload(c, "./contracts/testing/dummyTicks.js")
            )
          )
        )
      );
      ...
      assert.equal(
        JSON.stringify(tickedActions),
        JSON.stringify(expectedTickedActions)
      );
      ...
      assert.equal(
        JSON.stringify(tickedActions),
        '["tokens.checkPendingUnstakes","tokens.checkPendingUndelegations","nft.checkPendingUndelegations","botcontroller.tick","mining.checkPendingLotteries","airdrops.checkPendingAirdrops","nftauction.updateAuctions","witnesses.scheduleWitnesses","tokenfunds.checkPendingDtfs"]'
      );
    ...
    new Promise(async (resolve) => {
      ...
      transactions.push(
        new Transaction(
          ...
          "update",
          JSON.stringify(
            setupContractPayload(
              "testtick",
              "./contracts/testing/dummyTicks.js"
            )
          )
        )
      );
      ...
      assert.equal(
        txs[1].logs,
        `{"events":[{"contract":"contract","event":"registerTick","data":{"contract":"testtick","action":"tick","startRefBlock":${
          refBlockNumber + 1
        }}}]}`
      );

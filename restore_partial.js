...
program
  ...
  .option(
    "-s, --snapshot-url [url]",
    "base directory of light node snapshots to download",
...
/**
 * Updates the @startSteemBlock in the config.json file to the @newStartBlock.
 * @param newStartBlock to set in the config.json
 */
async function updateConfigJson(newStartBlock) {
  const config = fs.readJSONSync("./config.json");
  config.startSteemBlock = newStartBlock;
  fs.writeJSONSync("./config.json", config, { spaces: 4 });
  console.log(`set config.json startSteemBlock to ${newStartBlock}`);
}
...
async function execMongorestore(archiveName) {
  console.log(
    `starting restore using 'mongorestore --quiet --gzip --archive=${archiveName}'`
  );
  console.log("this will take 30 to 60 minutes without any log output...");

  exec(
    `mongorestore${drop ? "" : " --quiet"} --gzip --archive=${archiveName}`,
    (error, stdout, stderr) => {
      if (error) {
...
async function revertDatabase(
  ...
  const refHiveBlockDiff =
    divergentBlock.refSteemBlockNumber - archiveHiveBlock;
  ...
  console.log(
    `Divergent block: ${divergentBlockNum} (${divergentBlock.refSteemBlockNumber}) Archive block: ${archiveBlockNum} (${archiveHiveBlock}) Restoring to: ${deleteFromBlock}`
  );

  const collectionsToRemove = await database.database
    .listCollections()
    .toArray();
  ...
  await database.database
    ...
    .deleteMany({ blockNumber: { $gte: deleteFromBlock } });
}
...
async function restorePartial() {
  ...
  if (lightNode && !drop) {
    console.log(
      "Restoring a light node database is not supported. Please add the '-d' flag to completely restore your db."
    );
    return;
  }
  ...
  if (!archive || typeof archive !== "string") {
    ...
    if (!archive) {
      console.log(
        "start program with 'node restore_partial.js --archive <archive name>' or add a valid snapshot-url"
      );
      return;
    }
  }

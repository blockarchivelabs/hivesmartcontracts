...
const revertCommentsContractPayload = setupContractPayload(
  "comments",
  "./contracts/revert/comments_minify_20211027.js"
);
...
class Block {
  ...
  calculateMerkleRoot(transactions) {
    ...
    for (let index = 0; index < nbTransactions; index += 2) {
      ...
      const right =
        index + 1 < nbTransactions ? tmpTransactions[index + 1].hash : left;

      const leftDbHash = tmpTransactions[index].databaseHash;
      const rightDbHash =
        index + 1 < nbTransactions
  ...
  async blockAdjustments(database) {
    if (
      this.refSteemBlockNumber === 43447729 ||
      this.refSteemBlockNumber === 44870101
    ) {
    ...
    if (this.refSteemBlockNumber === 50352631) {
      const tokenBalances = database.database.collection("tokens_balances");
      await tokenBalances.updateOne(
        { _id: 8416 },
        { $set: { account: "nightowl1", balance: "1.05000000" } }
      );
      await tokenBalances.updateOne(
        { _id: 21725 },
        { $set: { account: "nightowl1", balance: "1010.00000000" } }
      );
    } else if (this.refSteemBlockNumber === 50354478) {
      const tokenBalances = database.database.collection("tokens_balances");
      await tokenBalances.updateOne(
        { _id: 8416 },
        { $set: { account: "nightowl1", balance: "0.50000000" } }
      );
    } else if (this.refSteemBlockNumber === 50354625) {
      const tokenBalances = database.database.collection("tokens_balances");
      await tokenBalances.updateOne(
        { _id: 21725 },
        { $set: { account: "nightowl1", balance: "500000.00000000" } }
      );
    }

    // Comments contract causing issues and needs to be reverted. put at beginning
    if (this.refSteemBlockNumber === 58637536) {
      this.transactions.unshift(
        new Transaction(
          this.blockNumber,
          "FIXTX_COMMENTS_REVERT",
          CONSTANTS.STEEM_ENGINE_ACCOUNT,
          "contract",
          "update",
          JSON.stringify(revertCommentsContractPayload)
        )
      );
    }
  }
  ...
  async produceBlock(database, jsVMTimeout, mainBlock) {
    ...
    let relIndex = 0;
    const allowCommentContract = this.refSteemBlockNumber > 54560500;
    for (let i = 0; i < nbTransactions; i += 1) {
      ...
      if (
        transaction.contract !== "comments" ||
        allowCommentContract ||
        transaction.logs === "{}"
      ) {
        if (
          mainBlock &&
    ...
    for (let i = 0; i < contractsConfig.contractTicks.length; i += 1) {
      ...
      if (this.refSteemBlockNumber >= contractTick.startRefBlock) {
        virtualTransactions.push(
          new Transaction(
            0,
            "",
            "null",
            contractTick.contract,
            contractTick.action,
            ""
          )
        );
    ...
    for (let i = 0; i < nbVirtualTransactions; i += 1) {
      ...
      if (
        ...
        for (let j = 0; j < contractsConfig.contractTicks.length; j += 1) {
          ...
          if (
            transaction.contract === contractTick.contract &&
        ...
        if (
          transaction.contract === "inflation" &&
    ...
    if (this.refSteemBlockNumber === 59376574) {
      this.virtualTransactions = this.virtualTransactions.concat(
        this.virtualTransactions
      );
    }
    ...
    if (this.transactions.length > 0 || this.virtualTransactions.length > 0) {
      ...
      const finalTransactions = this.transactions.concat(
        this.virtualTransactions
      );
      ...
      await database.noteHashChange(this.refSteemBlockNumber);
    }
  }
  ...
  async processTransaction(
    ...
    if (sender && contract && action) {
      if (
        ...
        const authorizedAccountContractDeployment = [
          "null",
          CONSTANTS.STEEM_ENGINE_ACCOUNT,
        ...
        if (authorizedAccountContractDeployment.includes(sender)) {
          results = await SmartContracts.deploySmartContract(
            // eslint-disable-line
            database,
            transaction,
            this.blockNumber,
          ...
          results = {
            logs: {
              errors: ["the contract deployment is currently unavailable"],
            },
          };
        }
      } else if (
        ...
        results = await SmartContracts.executeSmartContract(
          // eslint-disable-line
          database,
          transaction,
          this.blockNumber,
      ...
      results = {
        logs: {
          errors: ["the parameters sender, contract and action are required"],
        },
      };
    }
    if (
      results.logs &&
      results.logs.errors &&
      results.logs.errors.find((m) => m.includes("MongoError"))
    ) {
      throw new Error(
        `Mongo tx error, transaction: ${JSON.stringify(
          transaction

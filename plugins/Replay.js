    ...
    if (!err && stats.isFile()) {
      ...
      lr.on("line", async (line) => {
        ...
        if (line !== "") {
          ...
          if (blockNumber !== 0) {
            currentHiveBlock = refSteemBlockNumber;
            currentBlock = blockNumber;
            console.log(`replaying block ${currentBlock} / ${lastBockNumber}`); // eslint-disable-line no-console

            if (hiveClient !== null && finalrefSteemBlockId === undefined) {
              const hiveBlock = await hiveClient.database.getBlock(
                refSteemBlockNumber
              );
              finalrefSteemBlockId = hiveBlock.block_id;
              finalprevRefSteemBlockId = hiveBlock.previous;
            }

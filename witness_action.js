...
const privateSigningKey = dhive.PrivateKey.fromString(
  process.env.ACTIVE_SIGNING_KEY
);
...
program
  ...
  .option(
    "-e, --engineNode [url]",
    "verify with given hive-engine node",
    "https://localhost/rpc"
  )
    ...
    .then((res) => {
      if (res.data.result) {
        ...
        if (parsedLogs.errors) {
          ...
          console.log(
            `Failed action with reason "${parsedLogs.errors[0]}", your trx id: ${trxID}`
          );
        } else {
          // eslint-disable-next-line no-console
        ...
        if (tries >= 4) {
          // eslint-disable-next-line no-console
          console.log(
            `Successful broadcast but could not verify, trx id: ${trxID}`
          );
          return;
        }
        setTimeout(() => {
...
function broadcastWitnessAction(contractAction, contractPayload) {
  ...
  client.broadcast
    .json(transaction, privateSigningKey)
    .then((res) => {
      if (verify) {
...
program.command("register").action(() => {
  if (!skipDiverganceCheck) {
    exec(`node find_divergent_block.js -h -n ${engineNode}`).on(
      "exit",
      (code) => {
        if (code != 0) {
          // eslint-disable-next-line no-console
          console.log(
            `A divergent block was found, not registering. Run node find_divergent -n ${engineNode} to learn where.`
          );
          return;
        } else {
...
program.command("unregister").action(() =>
  broadcastWitnessAction("register", {
    IP: ip,
    RPCPort: extRPCNodePort,
    P2PPort: extP2PPort,
